...
//!
//! Uses `apeireth-http-client` for the actual HTTP GET, then extracts
//! accessibility tree from the response body. No Chrome needed.
//!
//! v2.0 适配: v2 `apeireth-http-client::HttpClient::new(base_url)` 返 Self (非 Result),
//! response 是 { status: u16, headers, body: String } struct (无 status()/text() 方法).
//! 0 装 PASS: HTTP 走 v2 http-client (master working tree 是 stub), title/ARIA 仍真跑.
...
impl FetchBrowser {
    pub fn new() -> Result<Self, BrowserError> {
        // v2 http-client 不需 base_url (传空); v2 stub 行为 OK
...
impl Browser for FetchBrowser {
    async fn navigate(&self, url: &str) -> Result<PageSnapshot, BrowserError> {
        ...
        // v2 HttpResponse: status 是 u16 字段, body 是 String 字段
