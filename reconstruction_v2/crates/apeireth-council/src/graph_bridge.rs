...
//!
//! **目标**: cognition graph 跑完后, summary 自动注入 council deliberation 的 query context,
//! 让 advisor 拿 24 维真实值做审议.
//!
//! **v2 适配**:
//! v1 依赖 `apeireth_graph::cognition_graph::CognitionSummary`. v2 没有 apeireth-graph crate.
//! 在本模块本地定义等价的 `CognitionSummary` struct (v1 字段完整保留), 让 bridge 保持自包含可用.
//! 等未来 v2 加回 apeireth-graph 时, 可以把本地定义改为 re-export.
//!
...
///
/// v1 字段完整保留 (`mean` / `min` / `max` / `verdict_approve` / `node_count`),
/// 公共 API 与 v1 一致, 任何 caller 都可以 0 改代码接入.
...
impl CognitionSummary {
    pub fn new(mean: f64, min: f64, max: f64, verdict_approve: bool, node_count: usize) -> Self {
        Self { mean, min, max, verdict_approve, node_count }
    }
}
