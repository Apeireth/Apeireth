//! g5_council_bridge - council deliberation 5 步映射到 pipeline_runtime 通用 5 阶段 substrate
//!
...
use crate::pipeline_runtime::{Pipeline, PipelineConfig, PipelineMessage, Stage, StageKind};
...
impl Stage<PipelineMessage, PipelineMessage> for CouncilDispatchStage {
    ...
    ) -> Result<PipelineMessage, crate::pipeline_runtime::PipelineError> {
        let mut m = input;
        if m.kind.is_empty() {
...
impl Stage<PipelineMessage, PipelineMessage> for CouncilReliabilityStage {
    ...
    fn process(
        ...
        if self.state.is_suppressed(&fp) {
            m.attempt += 1;
            return Err(crate::pipeline_runtime::PipelineError::Stage {
