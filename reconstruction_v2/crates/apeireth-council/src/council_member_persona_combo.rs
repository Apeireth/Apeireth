...
use crate::persona::{Persona as PersonaTrait, PersonaImpl};
pub type Persona = PersonaImpl;
...
impl PersonaBoundMember {
    ...
    pub fn initial_stance_kind(&self) -> StanceKind {
        crate::persona::initial_stance_kind_from_bias(self.persona.stance_bias)
    }
...
impl PersonaBoundDeliberator {
    ...
    pub fn with_llm_provider(
        mut self,
        llm: std::sync::Arc<dyn apeireth_asi::llm_judge::LlmProvider>,
    ) -> Self {
        ...
        for round in 0..self.max_rounds {
            ...
            for (idx, pbm) in self.members.iter().enumerate() {
                ...
                let (stance_kind, confidence, text) = if let Some(llm) = &self.llm {
                    ...
                    let kind = crate::persona::initial_stance_kind_from_bias(pbm.persona.stance_bias);
