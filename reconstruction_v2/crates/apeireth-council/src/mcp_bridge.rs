...
//! - `CouncilPromptServer` impl ``mcp_runtime::prompts::PromptServer``
//!   - 1 个 prompt: `council_deliberate` (params: query, area?, risk_level?)
//!   - 返 messages[]: system (council 介绍) + user (query) + 7 advisor role 引述
//! - `CouncilResourceServer` impl ``mcp_runtime::resources::ResourceServer``
//!   - 3 resources:
...
//! - 0 改 `mcp_runtime::prompts` / `mcp_runtime::resources` 已有类型 (R33-3 / R84 LOCKED)
...
use crate::mcp_runtime::resources::{Resource, ResourceContent, ResourceServer, RESOURCE_NOT_FOUND};
...
mod tests {
    use super::*;
    use crate::mcp_runtime::prompts::PROMPT_NOT_FOUND;
    use crate::mcp_runtime::resources::RESOURCE_NOT_FOUND;
    ...
    fn prompt_get_with_query() {
        ...
        assert_eq!(
            ...
            crate::mcp_runtime::prompts::PromptRole::Assistant
        );
        // Second message: user (the query)
        assert_eq!(
            result.messages[1].role,
            crate::mcp_runtime::prompts::PromptRole::User
        );
    }
    ...
    fn prompt_get_includes_members_in_system() {
        ...
        match &result.messages[0].content {
            crate::mcp_runtime::prompts::PromptContent::Text { text, .. } => {
