...
use crate::v1_era::normalized::NormalizedRequest;
...
impl ProtocolBridge for OpenClawGatewayBridge {
    ...
    async fn handle(&self, req: NormalizedRequest) -> Result<NormalizedResponse, ProtocolError> {
        ...
        let last_user = req
            ...
            .iter()
            .rev()
            .find(|m| matches!(m.role, crate::v1_era::normalized::MessageRole::User))
            .map(|m| crate::v1_era::normalized::ContentPart::join_text(&m.content))
            .unwrap_or_default();
