...
mod tests {
    ...
    #[test]
    fn html_escape_replaces_all_five() {
        assert_eq!(html_escape("a&b<c>d\"e'f"), "a&amp;b&lt;c&gt;d&quot;e&#39;f");
    }

    #[test]
    fn html_escape_idempotent_on_plain_text() {
        assert_eq!(html_escape("hello world"), "hello world");
    }

    #[test]
    fn html_escape_handles_empty() {
        assert_eq!(html_escape(""), "");
    }

    #[test]
    fn render_error_page_contains_msg() {
        let page = render_error_page("something failed");
        assert!(page.contains("something failed"));
        assert!(page.contains("Apeireth 错误"));
        assert!(page.contains("← 返回首页"));
    }
}
