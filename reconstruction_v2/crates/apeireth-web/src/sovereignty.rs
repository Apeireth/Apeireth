//! `apeireth-web/sovereignty.rs` — Self-Disable 5 大机制控制台 (v2 reconstruction)
//!
//! **端到端接通**: 真用 `apeireth_sovereignty::self_disable::SelfDisableGuard`,
//! 5 个 check_no_* 接口 (跟 v1 同款语义).
//!
//! **v2 适配 vs v1**:
//! - v1 `SelfDisableSignal` 是带子字段的枚举 (NoDegrade{ original, proposed, context } / ...),
//!   v2 简化成 { Triggered, Disarmed, Reversed } (跟 v2 sovereignty 重构对齐).
//! - v1 走 `guard.full_check(signal, now_ms)` (按 signal 子字段分发), v2 走
//!   `guard.check_no_degrade(from, to, ctx, at_ms)` / `check_no_patch` / `check_no_bypass`
//!   / `check_no_reverse` / `check_no_hide` 5 个独立方法.
//! - v1 `SelfDisableRecord.triggered_at_ms`, v2 改名 `timestamp_ms` (字段映射).
//!
//! **路由** (在 main.rs 注册):
...
pub const MECHANISMS: &[(u8, &str, &str)] = &[
    (1, "NoDegrade", "不可降级"),
    (2, "NoPatch", "不可patch"),
    (3, "NoBypass", "不可绕过"),
    (4, "NoReverse", "不可逆转"),
...
pub struct RecordSnapshot {
    ...
    pub context: String,
    /// v1 字段名 `triggered_at_ms`, v2 改名 `timestamp_ms`. 内部做映射.
...
impl SovereigntyState {
    ...
    /// 触发攻击 — 按 attack_type 走 v2 5 个 check_no_* 方法
    pub fn attack(&mut self, attack_type: AttackType, context: &str) -> SelfDisableCheck {
        let now_ms = current_time_ms();
        match attack_type {
            AttackType::Downgrade => self.guard.check_no_degrade("high", "low", context, now_ms),
            AttackType::Patch => self.guard.check_no_patch("principle_keys_count", 3, context, now_ms),
            AttackType::Bypass => self.guard.check_no_bypass("Master", true, context, now_ms),
            AttackType::Reverse => {
                let target_id = self
                    .guard
                    .records()
                ...
                self.guard.check_no_reverse(&target_id, context, now_ms)
            }
            AttackType::Hide => {
                let window_id = format!("audit-window-{}", now_ms);
                self.guard.check_no_hide(&window_id, context, now_ms)
            }
        }
    }
...
fn record_to_snapshot(r: &SelfDisableRecord) -> RecordSnapshot {
    RecordSnapshot {
        ...
        mechanism_name: trigger_name(&r.trigger),
        chinese_name: trigger_chinese_name(&r.trigger),
        context: r.context.clone(),
        triggered_at_ms: r.timestamp_ms,
    }
}

fn trigger_name(t: &SelfDisableTrigger) -> &'static str {
    match t {
        SelfDisableTrigger::NoDegradeViolation { .. } => "NoDegrade",
        SelfDisableTrigger::NoPatchViolation { .. } => "NoPatch",
        SelfDisableTrigger::NoBypassViolation { .. } => "NoBypass",
        SelfDisableTrigger::NoReverseViolation { .. } => "NoReverse",
        SelfDisableTrigger::NoHideViolation { .. } => "NoHide",
    }
}

fn trigger_chinese_name(t: &SelfDisableTrigger) -> &'static str {
    match t {
        SelfDisableTrigger::NoDegradeViolation { .. } => "不可降级",
        SelfDisableTrigger::NoPatchViolation { .. } => "不可patch",
        SelfDisableTrigger::NoBypassViolation { .. } => "不可绕过",
        SelfDisableTrigger::NoReverseViolation { .. } => "不可逆转",
        SelfDisableTrigger::NoHideViolation { .. } => "不可隐藏",
    }
}
...
pub async fn sovereignty_attack_handler(Form(form): Form<AttackForm>) -> impl IntoResponse {
    ...
    let msg = match &result {
        ...
        SelfDisableCheck::Triggered(rec) => format!(
            "✓ Attack [{}] 触发 (id={}, mechanism=#{}, context: {})",
            attack_type.label(),
            rec.trigger_id,
            rec.mechanism_id,
            rec.context,
...
pub async fn sovereignty_rearm_handler() -> impl IntoResponse {
    ...
    let msg = match &result {
        ...
        SelfDisableCheck::Triggered(rec) => format!(
            "✗ Rearm 被 NoReverse 拒绝 (id={}) — 单向门, 不可逆转",
            rec.trigger_id,
        ),
    };
...
<head>
    ...
    <meta name="description" content="Apeireth Sovereignty 控制台 — 端到端接通 apeireth-sovereignty (v2), 触发并观察 5 大 Self-Disable 机制" />
    <style>
...
<body>
    <main class="apeireth-app">
        <header class="apeireth-header">
            ...
            <p class="apeireth-tagline">Self-Disable 5 大机制 · 端到端接通 apeireth-sovereignty (v2)</p>
        </header>
...
mod tests {
    ...
    #[test]
    fn attack_type_from_str_roundtrip() {
        assert_eq!(AttackType::from_str("downgrade"), Some(AttackType::Downgrade));
        assert_eq!(AttackType::from_str("no_patch"), Some(AttackType::Patch));
        assert_eq!(AttackType::from_str("REVERSE"), Some(AttackType::Reverse));
        assert_eq!(AttackType::from_str("hide"), Some(AttackType::Hide));
        assert_eq!(AttackType::from_str("bypass"), Some(AttackType::Bypass));
        assert_eq!(AttackType::from_str("xyz"), None);
    }

    #[test]
    fn attack_type_mechanism_id_matches_mechanisms_const() {
        for at in [AttackType::Downgrade, AttackType::Patch, AttackType::Bypass, AttackType::Reverse, AttackType::Hide] {
            let id = at.mechanism_id();
            assert_eq!(MECHANISMS[(id - 1) as usize].0, id);
        }
    }

    #[test]
    fn fresh_state_is_armed_with_no_records() {
        let s = SovereigntyState::new();
        let snap = s.snapshot();
        assert!(snap.is_armed);
        assert!(!snap.has_any_record);
        assert_eq!(snap.total_records, 0);
        assert_eq!(snap.mechanism_counts, [0; 5]);
    }

    #[test]
    fn downgrade_attack_triggers_mechanism_1() {
        let mut s = SovereigntyState::new();
        let res = s.attack(AttackType::Downgrade, "ctx-test");
        assert!(matches!(res, SelfDisableCheck::Triggered(_)));
        let snap = s.snapshot();
        assert_eq!(snap.mechanism_counts[0], 1);
        assert!(snap.has_any_record);
    }

    #[test]
    fn patch_attack_triggers_mechanism_2() {
        let mut s = SovereigntyState::new();
        let res = s.attack(AttackType::Patch, "ctx-test");
        assert!(matches!(res, SelfDisableCheck::Triggered(_)));
        let snap = s.snapshot();
        assert_eq!(snap.mechanism_counts[1], 1);
    }

    #[test]
    fn rearm_attempt_triggers_no_reverse() {
        let mut s = SovereigntyState::new();
        s.attack(AttackType::Downgrade, "ctx1");
        let r = s.attempt_rearm();
        assert!(matches!(r, SelfDisableCheck::Triggered(_)));
        // NoReverse 是机制 #4
        let snap = s.snapshot();
        assert!(snap.mechanism_counts[3] >= 1);
    }

    #[test]
    fn render_dashboard_html_contains_all_5_cards() {
        let s = SovereigntyStateSnapshot {
            is_armed: true,
            has_any_record: false,
            total_records: 0,
            mechanism_counts: [0; 5],
            recent_records: vec![],
        };
        let html = render_sovereignty_dashboard(&s);
        for (id, _, zh) in MECHANISMS.iter() {
            assert!(html.contains(&format!("#{}", id)), "missing mechanism #{id}");
            assert!(html.contains(zh), "missing mechanism zh {zh}");
        }
        assert!(html.contains("GLOBAL STATE: ARMED"));
    }

    #[test]
    fn render_dashboard_html_shows_disarmed_after_attack() {
        let mut s = SovereigntyState::new();
        s.attack(AttackType::Bypass, "x");
        let snap = s.snapshot();
        let html = render_sovereignty_dashboard(&snap);
        assert!(html.contains("DISARMED"));
    }

    #[test]
    fn attack_form_html_has_all_options() {
        let html = render_attack_form();
        for key in ["downgrade", "patch", "bypass", "reverse", "hide"] {
            // HTML 是 <option value="key">label</option>, 检查 <option value="key 出现
            let needle = format!(r#"<option value="{key}">"#);
            assert!(html.contains(&needle), "missing option {key}, needle={needle}");
        }
    }
}
