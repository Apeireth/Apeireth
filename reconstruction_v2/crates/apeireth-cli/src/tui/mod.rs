...
use self::state::{ActiveTab, AppState, FocusPane, ChatMessageItem};
...
pub async fn run_tui(api_key: String, db_path: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // 2. Setup Panic Hook to ensure terminal is restored if panic occurs
    ...
    // 3. Initialize Host & Load Initial Memories
