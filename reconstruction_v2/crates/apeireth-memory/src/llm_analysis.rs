...
//!
//! v2 适配: v2 apeireth-llm-iface 用 ChatRequest/ChatResponse (跟 v1 LlmRequest 不同),
//! 本文件直接用 ChatRequest + ChatMessage + LlmProvider::chat。

use std::sync::Arc;

use crate::compat::Episode;
use apeireth_llm_iface::{ChatMessage, ChatRequest, LlmProvider}; // v2 llm-iface surface
...
pub async fn analyze_episode(
    ...
    // v2 llm-iface 用 ChatRequest (model + messages + max_tokens + temperature)
    let req = ChatRequest {
        model: "apeireth-memory".into(),
        messages: vec![
            ChatMessage { role: "system".into(), content: kind.system_prompt().into() },
