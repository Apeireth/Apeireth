...
use crate::compat::{IdentityCard, Migration};
...
impl IdentityCardRecord {
    ...
    pub fn into_core(&self) -> IdentityCard {
        // v2 IdentityCard 还有 name / version / philosophy_anchors / created_at 字段
        // (#[serde(default)] 兼容 v1). into_core 用 apeireth_core::IdentityCard::default()
        // 起手, 然后覆盖 v1 字段 (保留 v2 默认).
        let mut card = IdentityCard::default();
        card.continuity_id = self.continuity_id.clone();
        card.birth_time = self.birth_time;
        card.carriers = self.carriers.clone();
        card.migration_history = self.migration_history.clone();
        card
    }
}
