...
fn r177_asi_05_v05_categories() {
    ...
    let continuity_count = names.iter().filter(|n| {
        n.contains("continuity") || n.contains("recall") || n.contains("context_window")
            || n.contains("session_recovery") || n.contains("identity_persistence")
    }).count();
    let salience_count = names.iter().filter(|n| {
        n.contains("importance") || n.contains("novelty") || n.contains("actionability")
            || n.contains("confidence") || n.contains("temporal_relevance")
    }).count();
    let identity_count = names.iter().filter(|n| {
        n.contains("core_values") || n.contains("voice_") || n.contains("behavioral")
            || n.contains("role_") || n.contains("philosophy_alignment")
    }).count();
    let philosophy_count = names.iter().filter(|n| {
        n.starts_with("v1_") || n.starts_with("v2_") || n.starts_with("v3_")
            || n.contains("cone_of_truth") || n.contains("action_guard")
    }).count();
...
fn r177_asi_06_v05_scores_struct() {
    let s = AsiV05Scores { continuity: 0.5, salience: 0.5, identity: 0.5, philosophy_guard: 0.5, transferability: 0.5 };
...
fn r177_asi_07_v1136_struct() {
    let s = V1136Submeasures { continuity_5: [0.5; 5], transferability_2: [0.5; 2] };
...
fn r177_asi_09_v1136_default() {
    ...
    for v in s.continuity_5.iter() { assert_eq!(*v, 0.0); }
    for v in s.transferability_2.iter() { assert_eq!(*v, 0.0); }
}
