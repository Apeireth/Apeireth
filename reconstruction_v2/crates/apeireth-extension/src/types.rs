...
mod tests {
    ...
    #[test]
    fn all_kinds_as_str_round_trip() {
    ...
    fn parse_accepts_legacy_aliases() {
        assert_eq!(PluginKind::parse("messagePreprocessor"), Some(PluginKind::MessagePreprocessor));
        assert_eq!(PluginKind::parse("preprocessor"), Some(PluginKind::MessagePreprocessor));
    }

    #[test]
    fn parse_unknown_returns_none() {
        assert!(PluginKind::parse("alien").is_none());
        assert!(PluginKind::parse("").is_none());
    }

    #[test]
    fn audit_entry_success_has_no_error() {
        let e = AuditEntry::success("p", PluginKind::Sync, 10, 20, 100);
        assert!(e.success);
        assert!(e.error.is_none());
        assert_eq!(e.output_bytes, 20);
        assert!(!e.trace_id.is_empty());
    }

    #[test]
    fn audit_entry_failure_has_error_zero_output() {
        let e = AuditEntry::failure("p", PluginKind::Async, 10, 50, "boom");
        assert!(!e.success);
    ...
    #[test]
    fn plugin_kind_serde_snake_case() {
        let json = serde_json::to_string(&PluginKind::MessagePreprocessor).unwrap();
        assert_eq!(json, "\"message_preprocessor\"");
        let parsed: PluginKind = serde_json::from_str("\"hybrid\"").unwrap();
        assert_eq!(parsed, PluginKind::Hybrid);
    }
}
