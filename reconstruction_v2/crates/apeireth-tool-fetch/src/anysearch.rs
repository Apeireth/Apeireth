...
impl AnySearchClient {
    ...
    async fn call(&self, tool_name: &str, arguments: Value) -> AnySearchResult<Value> {
        ...
        // v2 compat: use HttpClient::post (stub returning mock JsonRpc response).
        let resp = self.http.post(&self.endpoint, body.clone()).await
            .map_err(|e| AnySearchError::Transport(format!("send: {e}")))?;
        let status = resp.status_code();
        if status != 200 {
            return Err(AnySearchError::Transport(format!("HTTP {status}")));
        }
        let parsed: JsonRpcResponse = serde_json::from_str(resp.body_text())
