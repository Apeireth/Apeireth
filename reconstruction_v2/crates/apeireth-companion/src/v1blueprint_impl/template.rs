...
//! - O-2 主 19:33 — 借鉴 v0.9.21 6 模板 1:1 翻译.
...
use super::error::{BlueprintError, BlueprintResult};
...
pub trait Logging: Send + Sync {
    ...
    fn audit_log(&self, event: &super::risk::AuditEvent) -> BlueprintResult<()>;
}

/// 模板 F 默认实现 — tracing 走 tracing crate, audit 走 InMemoryAudit.
#[derive(Default)]
pub struct TracingAuditLog {
    audit: super::risk::InMemoryAudit,
}
...
impl Logging for TracingAuditLog {
    ...
    fn audit_log(&self, event: &super::risk::AuditEvent) -> BlueprintResult<()> {
        ...
        super::risk::K3Audit::audit(&self.audit, event)
    }
}
