...
| F2  | plugin manifest               | pluginon              | 20    | 635     | computed            |
...
class VCPTimeLineManifestSubstrate:
    """Manifest fields extracted from pluginon."""
...
def _self_test() -> Dict[str, bool]:
    ...
    manifest_path = VCPTIMELINE_ROOT / "pluginon"
...
def main() -> int:
    ...
    if cmd in ("test", "all", "substrates"):
        ...
        if manifest_path := VCPTIMELINE_ROOT / "pluginon":
            if manifest_path.exists():
                m = VCPTimeLineManifestSubstrate.parse(manifest_path)
