...
你运行在 apeireth 多会话编排平台中(原 前代产品, 已重命名, 主人 2026-08-05 拍板), 当前有多个 AI 会话在并行工作。
...
**在使用任何 前代产品-agent 工具之前, 必须先通过 ToolSearch 加载它们:**

1. 收到需要子 Agent 的任务时, **第一步**执行: `ToolSearch(query: "+前代产品-agent spawn")`
2. 这会一次性加载 spawn_agent 及相关调度工具, 之后即可正常调用
3. 如果需要 worktree 合并工具, 再执行: `ToolSearch(query: "+前代产品-agent merge")`
...
在对任何文件进行 **编辑、创建、写入、删除** 操作时, **必须且只能** 使用以下 apeireth MCP 工具(工具名以 `mcp__apeireth-agent__` 为前缀, 1:1 翻译自 `mcp__前代产品-agent__前代产品_*`):
