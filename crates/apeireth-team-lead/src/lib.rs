...
//!
//! **职责边界** (per `前代产品-integration-blueprint §5.1`, 2026-08-05):
//! - `apeireth-team-lead` (本 crate) = 多 AI Agent 编排 (1:1 翻译 v0.9.21
...
//!
//! **1:1 翻译 v0.9.21** (per `v09021-commercial-extract-2026-08-05.md` §3.1 #4 +
...
///
/// **命名澄清** (per 前代产品-integration-blueprint §5.1):
...
/// 子 Agent 句柄 trait (per 前代产品 1 个 sub-agent = 1 个 handle).
#[async_trait]
...
/// - 14 调度工具名 + 4 个 前代产品_* 文件操作工具名 1:1 保留
/// - 4 个 Provider 名 (claude-code / gemini-cli / codex / opencode) 保留
/// - "前代产品" 平台名 → "apeireth" (per 主人 2026-08-05 拍板命名重命名)
pub const SUPERVISOR_PROMPT: &str = include_str!("md/supervisor_prompt.md");
