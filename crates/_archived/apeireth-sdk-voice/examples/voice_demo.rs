...
async fn main() {
    ...
    // 2) 4 STT 模型
    println!("[§2 4 STT 模型 (per v0.9.21 + task spec §3)]");
    ...
    // 3) 4 TTS 模型
    println!("[§3 4 TTS 模型 (per v0.9.21 + task spec §3)]");
    ...
    // 4) 4 唤醒词类别
    println!("[§4 4 唤醒词类别 (per v0.9.21 + task spec §3, 默认 Hardcoded + 'apeireth')]");
    ...
    // 5) 3 VAD 算法
    println!("[§5 3 VAD 算法 (per v0.9.21 + task spec §3)]");
