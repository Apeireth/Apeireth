...
//! - `pluginon:55 ReadFile` → `FileOps::read`
//! - `pluginon:63 WriteFile` → `FileOps::write`
//! - `pluginon:79 ListDirectory` → `FileOps::list`
//! - `pluginon:99 DeleteFile` → `FileOps::delete`
//! - `pluginon:91 MoveFile` → `FileOps::move_path`
//! - `pluginon:103 CreateDirectory` → `FileOps::mkdir`
//! - VCP `WebReadFile` (pluginon:59) HTTP GET 模式 → `WebSearch` trait
...
pub use guardrail::{post_call_tripwire, pre_call_guard, GuardrailError, GuardrailKind, Tripwire};
pub use register::{
    default_schema_map, register_all, registered_tool_names, REGISTERED_TOOL_COUNT, TOOL_NAMES,
};
