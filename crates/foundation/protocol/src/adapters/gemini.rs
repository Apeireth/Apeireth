...
//! - **`tools[].functionDeclarations[]`** 数组 (复刻 既有实现 真机制)
...
impl ProtocolAdapter for GeminiAdapter {
    ...
    fn adapt_request(&self, req: &NormalizedRequest) -> Result<Value, ProtocolError> {
        ...
        // 设计参考 system → systemInstruction 顶层
        ...
        // 映射设计参考 (protocolBridge.js:91-118):
        ...
        // 映射设计参考 (protocolBridge.js:120-156):
        ...
        // 设计参考 legacy functions (protocolBridge.js:110-115): 兼容旧 Gemini
                    ...
                    for part in parts {
                        ...
                        if let Some(fc) = part.get("functionCall") {
                            let name = fc
                                ...
                                .unwrap_or("")
                                .to_string();
                            // 设计参考 Gemini functionCall 没有 id, 用 name 替代 (保持唯一性靠 user 端)
...
mod tests {
    ...
    fn request_tools_use_function_declarations() {
        // 设计参考 protocolBridge.js:91-118 extractProtectedTools 真机制
    ...
    fn request_tool_choice_specific_uses_allowed_function_names() {
        // 设计参考 normalizeToolChoice (protocolBridge.js:120-156): mode=ANY + allowedFunctionNames
        let mut req = NormalizedRequest::new("gemini-1.5-pro", vec![NormalizedMessage::user("hi")]);
    ...
    fn request_assistant_tool_calls_become_function_call_parts() {
        // 设计参考 OpenAI tool_calls → Gemini functionCall parts
        let tc = ToolCall {
            id: "call_1".into(),
            name: "get_weather".into(),
    ...
    fn request_tool_message_becomes_user_function_response() {
        // 设计参考 OpenAI tool → Gemini user + functionResponse
        let req = NormalizedRequest::new(
            "gemini-1.5-pro",
