...
//! - 字段命名约定: `output_text` 顶层 + `output[0].content[0].text` (既有实现)
...
impl ProtocolAdapter for OpenAiResponsesAdapter {
    ...
    fn adapt_request(&self, req: &NormalizedRequest) -> Result<Value, ProtocolError> {
        ...
        // 映射设计参考 buildImmediateResponsesPayload:
        ...
        // parallel_tool_calls 设计参考 protocolBridge.js:169-171
...
mod tests {
    ...
    fn request_system_promoted_to_instructions() {
        // 设计参考 protocolBridge.js:210-220: system → 顶层 instructions
    ...
    fn request_input_uses_input_text() {
        // 设计参考 input item 的 content 用 input_text 不是 text
    ...
    fn request_tool_as_function_call_output() {
        // 设计参考 tool message → function_call_output
        let req = NormalizedRequest::new(
            "gpt-4o",
            vec![
    ...
    fn request_parallel_tool_calls_via_metadata() {
        // 设计参考 protocolBridge.js:169-171
        let mut req = NormalizedRequest::new("gpt-4o", vec![NormalizedMessage::user("hi")]);
        req.metadata
    ...
    fn response_with_output_array_message() {
        // 设计参考 buildImmediateResponsesPayload: output[0].content[0].text
        let raw = json!({
            "id": "resp_123",
            "object": "response",
    ...
    fn response_uses_top_level_output_text_when_no_output() {
        // 设计参考 buildImmediateResponsesPayload: 顶层 output_text 兜底
        let raw = json!({
            "id": "x", "model": "gpt-4o",
            "status": "completed",
