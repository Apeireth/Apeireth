...
impl MessageRole {
    ...
    pub fn from_legacy_value(s: &str) -> Self {
        match s {
            ...
            _ => Self::User, // 既有实现默认 fallback 是 user
        }
    }
}
...
impl ContentPart {
    ...
    /// 从 raw content (string or array) 归一化
...
/// 归一化 tool_choice (映射设计参考 真代码)
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "type", rename_all = "snake_case")]
pub enum NormalizedToolChoice {
    /// 让 LLM 自动决定
    Auto,
    /// 不允许调工具 (OpenAI "none" / Anthropic 不支持)
    None,
    /// 必须调工具 (any-of 语义: 0 or 2+ allowed names)
    Required,
    /// 必须调指定工具 (any-of 语义: 1 allowed name / OpenAI specific function)
    Specific {
        /// 工具名
        name: String,
    },
...
pub struct NormalizedRequest {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub stop: Vec<String>,
    /// 工具列表 (归一化, **设计参考 extractProtectedTools 不进 messages 原则**)
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub tools: Vec<NormalizedTool>,
    /// 工具选择策略
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_choice: Option<NormalizedToolChoice>,
    /// 元数据透传 (映射设计参考 思路, 任何返回新数组的步骤保留)
...
mod tests {
    ...
    fn message_role_from_legacy_value_developer_to_system() {
        // 设计参考 protocolBridge.js:47-52: developer → system
        assert_eq!(
    ...
    fn content_part_from_legacy_string() {
        // 设计参考 protocolBridge.js:21-42: string 原样
    ...
    fn content_part_from_legacy_value_array_text_types() {
        // 设计参考 protocolBridge.js:31-34: text / input_text / output_text 都归一化
