...
pub mod usage;
// R20 阶段 2: WebSocket 8 帧协议 (接口契约). 跟 4 LLM 协议归一化层并列, 0 冲突.
pub mod ws_v1;
...
/// 默认 OpenAI Chat 路径 (兼容 OpenAI 路径: `/v1/chat/completions`)
...
const _: () = {
    ...
    // Keep-Alive 5 字段 (设计参考 chatCompletionHandler.js:22-28)
    assert!(
        KEEP_ALIVE_KEEP_ALIVE,
        "Keep-Alive must be enabled (既有实现 keepAlive: true)"
    );
    assert!(
        KEEP_ALIVE_KEEP_ALIVE_MSECS >= 100,
        "Keep-Alive interval must be >= 100ms (既有实现: 1000ms)"
    );
    assert!(
        KEEP_ALIVE_FREE_SOCKET_TIMEOUT >= 1000,
        "Free socket timeout must be >= 1s (既有实现: 8000ms 防 zombie)"
    );
    assert!(
        KEEP_ALIVE_SCHEDULING_LIFO,
        "Scheduling must be LIFO (既有实现: 'lifo' 优先复用最新鲜连接)"
    );
    assert!(
        KEEP_ALIVE_MAX_SOCKETS >= 100,
        "Max sockets must be >= 100 (既有实现: 10000)"
    );
