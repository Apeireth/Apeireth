...
//!
//! **位置**: trait 在 `apeireth-plugin` (foundation), impl 留在 `crates/engine/perception` (engine).
//! 与 `MemoryBackend` / `Experience` / `CredentialResolver` 同位: 都是 capability 抽象.
//!
//! **0 装 PASS**:
...
//!
//! **3 阶审查** (O-6 锚 9):
//! 1. 总体: 与 MemoryBackend/Experience/CredentialResolver 4 件 capability 在 foundation 集中
//! 2. 系统: trait 在 foundation, impl 在 engine (单向, 与 plugin 体系一致)
//! 3. 架构: runtime 拿 `Arc<dyn PerceptionInput>`, 6 模 trait 抽象统一入口语义
//!
