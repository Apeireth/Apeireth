...
//!
//! 1. **4 块 1:1 翻译 v0.9.21 voice API**:
...
//!
//! - **S-1 北极星**: 1:1 翻译 v0.9.21 voice API 3 端点 URL (跟 OpenAI audio API
...
//!
//! 1. **唤醒词走 STUB hardcode**: v0.9.21 唤醒词由 Porcupine (本地唤醒词引擎)
...
/// 远端 voice API 响应外壳 (per v0.9.21 默认结构).
///
...
/// TTS 声音类型 (5 variant, 1:1 翻译v0.9.21 voice `voice_id` 枚举).
///
...
pub enum VoiceKind {
    /// `"apeireth-male"` (品牌男性, 1:1 翻译 v0.9.21, 编译期 hardcode).
    ApeirethMale,
    /// `"apeireth-female"` (品牌女性, 1:1 翻译 v0.9.21, 编译期 hardcode).
    ApeirethFemale,
...
/// STT 语言 (5 variant, 1:1 翻译v0.9.21 voice `lang` 枚举).
///
...
/// TTS 返 audio 格式 (3 variant, 1:1 翻译v0.9.21 voice `response_format` 枚举).
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
///
/// 字段对应 v0.9.21 voice buffer 6 字段:
...
/// duration_ms, format), 跟 v0.9.21 buffer 1:1 翻译. `AudioFrame` 是 STUB 路径
/// per-frame (Porcupine 512 frames), 留作 STUB, 不动.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AudioBuffer {
    /// 16-bit PCM samples (per v0.9.21).
    pub samples: Vec<i16>,
...
/// 唤醒词检测结果 (per v0.9.21 Porcupine 行为).
///
/// 字段:
...
/// 远端 401 错误码 (per v0.9.21 voice API 默认响应).
const VOICE_ERR_CODE_TOKEN_INVALID: i32 = 40101;
/// 远端 403 错误码 (per v0.9.21 voice API 默认响应).
...
/// 远端 voice API base URL (1:1 翻译v0.9.21 SDK 默认 endpoint).
/// 1:1 翻译 v0.9.21 voice SDK 默认 `https://api.apeireth.com/v1`.
...
impl VoiceRealImpl {
    ...
    ///
    /// 1:1 翻译v0.9.21 voice TTS 端点. 返 raw audio bytes (WAV / MP3 / PCM),
    ...
    ///
    /// 1:1 翻译v0.9.21 voice STT 端点 (跟 OpenAI Whisper API 同模式).
    ...
    ///
    /// 1:1 翻译v0.9.21 Porcupine 行为, 但**不**接 Porcupine SDK (per 0 重复造轮子 +
    ...
    ///
    /// 1:1 翻译v0.9.21 voice 声纹端点. 走 JSON 请求 + JSON 响应
