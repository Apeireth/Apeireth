...
//! 当前 crate 是 **STUB skeleton** — API 表面按 v0.9.21 Porcupine / pvrecorder 1:1
...
//! - 默认唤醒词: **"apeireth"** (1:1 翻译 v0.9.21品牌一致, **不写 Hey前代产品**)
//!
//! ## v0.9.21实查 (估 600 LOC, 估 8 工具)
//!
...
//
// 设计:
// - 4 块 1:1 翻译 v0.9.21 voice API: TTS / STT / 唤醒词 / 声纹
...
/// voice SDK schema version (v0.9.21 1:1, 暂不写 version.workspace).
/// K-1 强校验 #1: 编译期 hardcode, 不写 `"1"` 字符串 elsewhere.
pub const VOICE_SCHEMA_VERSION: &str = "1";

/// 平台名 (K-1 强校验 #2: 编译期 hardcode `"apeireth"`, v0.9.21 1:1 翻译, 不写 "前代产品" 等装饰名).
...
/// 默认采样率 (16kHz, per Porcupine 官方 + v0.9.21 pvrecorder).
pub const VOICE_SAMPLE_RATE_HZ: u32 = 16000;

/// 单帧长度 (512 frames, per Porcupine 官方 + v0.9.21 pvrecorder).
pub const VOICE_FRAME_LENGTH: u32 = 512;

/// 默认唤醒词 (K-1 强校验 #2 品牌一致: 编译期 hardcode `"apeireth"`, 不写 Hey前代产品).
pub const VOICE_DEFAULT_KEYWORD: &str = "apeireth";

/// 单次录音最大秒数 (per v0.9.21估 30s 限制, 防恶意录音占内存).
...
///
/// 字段对应 v0.9.21 Porcupine `keywords` 数组元素. 默认 = `Apeireth`.
...
pub enum WakeWordType {
    ...
    #[default]
    Apeireth,
    /// `"computer"` (经典 sci-fi 致敬, v0.9.21估 1:1).
    Computer,
    /// `"jarvis"` (经典 sci-fi 致敬, v0.9.21估 1:1).
    Jarvis,
    /// `"hey apeireth"` (长句, v0.9.21估 1:1).
    HeyApeireth,
...
impl WakeWordType {
    /// 唤醒词字符串 (跟 v0.9.21 Porcupine 关键词库 1:1).
...
/// STUB 模式不读, 但字段保留 1:1 翻译 v0.9.21 `accessKey` 配置.
...
pub struct AudioFrame {
    /// 16kHz / 16-bit PCM (per Porcupine 官方 + v0.9.21 pvrecorder).
    pub samples: Vec<i16>,
...
///
/// 字段对应 v0.9.21 Porcupine 初始化 (估 5 fields):
...
///
/// 字段对应 v0.9.21 pvrecorder (估 4 fields):
/// - `config` (per `VoiceConfig`)
...
///
/// 字段对应 v0.9.21 `voice` 模块 (估 3 fields):
/// - `config` (per `VoiceConfig`)
...
///
/// STUB 模式: trait 表面 1:1 翻译 v0.9.21, 但实现由调用方注入.
...
//   - audio thread pool (per v0.9.21 4 worker thread)
//   - keyword model cache (per v0.9.21 5 唤醒词预加载)
