//! `apeireth-provider::reasoning_adapter` — 推理字段归一化适配件 (N12 ②)
//!
//! **机制** (调研依据: 公开接口规范 +
...
//!    (转发给不支持推理字段的下游模型前使用)。
//!
//! **默认行为按既有约定**: enabled=false + 过滤器空 → 任何模型都不转换
//! (保守默认, 显式配置后才生效)。
//!
//! **0 假装**:
//! - 别名清单以公开接口 REASONING_KEYS 实数为准 (12 个); 台账写 "13 别名"
...
/// **片段级全局去重** (对既有实现的小改进: 原按整源文本去重, 流式场景 delta
...
/// 适配器配置 (配置项: ReasoningToContentEnabled / ReasoningToContentModel / Tag)。
#[derive(Debug, Clone)]
