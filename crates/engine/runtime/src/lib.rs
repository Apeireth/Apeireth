...
pub use canonical::{
    ...
    InMemorySessionStore, InvocationContext, InvocationOrigin, JudgeConfig, JudgeModule,
    JudgeObservations, JudgeResult, JudgeVerdict, McpModule, MemoryRecallModule,
    MemoryWritebackModule, Module, ModuleContext, ModuleDirective, ModuleError,
    ModuleInvocationError, ModuleInvocationRequest, ModuleInvocationResponse, ModuleInvoker,
    ModuleManifest, ModuleMetricsSnapshot, ModuleOutcome, ModuleRegistry, PendingApproval,
    PendingApprovalView, PreferenceRecallModule, ProductionBackends, ProductionCognitiveModules,
    ProductionModules, ProductionModulesConfig, PromptOverlay, ProviderHealth, ProviderRouter,
    RepoModule, RoutedCompletion, Runtime, RuntimeBuilder, RuntimeConfig, RuntimeError,
    RuntimeResult, RuntimeSubLoopSpawner, SearchModule, SelfAssessmentModule, Session,
    SessionEvent, SessionEventKind, SessionManager, SessionStore, ShellModule, SqliteSessionStore,
