...
impl StreamDepth {
    ...
    pub fn count_all(store: &SqliteMemoryStore, continuity_id: &str) -> MemoryResult<[u64; 6]> {
        let mut counts = [0; 6];
        for (index, kind) in StreamKind::ALL.iter().enumerate() {
            counts[index] = Self::count(store, *kind, continuity_id)?;
        }
        Ok(counts)
    }
    ...
    fn row_to_entry(row: &rusqlite::Row<'_>) -> rusqlite::Result<HistoryEntry> {
        ...
        Ok(HistoryEntry {
            ...
            source: row.get(6)?,
            tags: serde_json::from_str(&tags_text).map_err(|error| {
                rusqlite::Error::FromSqlConversionFailure(
                    7,
                    rusqlite::types::Type::Text,
                    Box::new(error),
...
mod tests {
    ...
    fn entry(id: &str, subject: &str, time: i64) -> HistoryEntry {
        HistoryEntry {
            id: id.into(),
    ...
    fn insert_and_count() {
        let db = store();
        StreamDepth::insert(&db, StreamKind::Thought, &entry("a", "c", 1)).unwrap();
        assert_eq!(
            StreamDepth::count(&db, StreamKind::Thought, "c").unwrap(),
            1
        );
    }
    #[test]
    fn query_is_newest_first() {
        let db = store();
        StreamDepth::insert(&db, StreamKind::Action, &entry("a", "c", 1)).unwrap();
        StreamDepth::insert(&db, StreamKind::Action, &entry("b", "c", 2)).unwrap();
        let rows = StreamDepth::query(&db, StreamKind::Action, "c", 10, None).unwrap();
        assert_eq!(rows[0].id, "b");
    }
    #[test]
    fn query_filters_since() {
        let db = store();
        StreamDepth::insert(&db, StreamKind::Action, &entry("old", "c", 1)).unwrap();
        StreamDepth::insert(&db, StreamKind::Action, &entry("new", "c", 2)).unwrap();
        assert_eq!(
            StreamDepth::query(&db, StreamKind::Action, "c", 10, Some(2))
                .unwrap()
                .len(),
            1
        );
    ...
    fn query_respects_limit() {
        ...
        for index in 0..3 {
            StreamDepth::insert(
                &db,
                StreamKind::Action,
                &entry(&index.to_string(), "c", index),
            )
            .unwrap();
        }
        assert_eq!(
            StreamDepth::query(&db, StreamKind::Action, "c", 2, None)
                .unwrap()
                .len(),
            2
        );
    ...
    #[test]
    fn count_all_tracks_each_kind() {
        let db = store();
        StreamDepth::insert(&db, StreamKind::Thought, &entry("t", "c", 1)).unwrap();
        StreamDepth::insert(&db, StreamKind::Reflection, &entry("r", "c", 1)).unwrap();
        assert_eq!(
            StreamDepth::count_all(&db, "c").unwrap(),
            [1, 0, 0, 0, 0, 1]
        );
    }
    #[test]
    fn query_by_name_resolves_kind() {
        let db = store();
        StreamDepth::insert(&db, StreamKind::Proposal, &entry("p", "c", 1)).unwrap();
        assert_eq!(
            StreamDepth::query_by_name(&db, "proposal", "c", 1, None)
                .unwrap()
                .len(),
            1
        );
    ...
    #[test]
    fn other_subjects_are_excluded() {
        let db = store();
        StreamDepth::insert(&db, StreamKind::Thought, &entry("a", "x", 1)).unwrap();
        assert!(StreamDepth::query(&db, StreamKind::Thought, "y", 10, None)
            .unwrap()
            .is_empty());
    }
}
