...
/// **默认 size 1024x1024** (per VCP `Plugin/DoubaoGen*/pluginon` 默认 1024)
...
impl GenPlugin {
    ...
    /// **plugin 中文描述** (per 07 §2 P2-13 列表 + VCP `Plugin/<X>/pluginon:description`)
...
/// - `prompt` — 文本 prompt (必填)
/// - `seed` — 可选 seed (per VCP `Plugin/FluxGen*/pluginon:seed`)
...
pub struct GenResponse {
    ...
    /// 实际用的 seed (per VCP `Plugin/FluxGen*/pluginon:seed_used`)
    #[serde(default, skip_serializing_if = "Option::is_none")]
