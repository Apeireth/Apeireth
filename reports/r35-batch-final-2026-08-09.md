...
| R33-1 | Aider 启动扫描项目结构 + VCP `Plugin/*/pluginon` 启动模式 | `conventions_scanner.rs` 文档注释 "Aider-style" + `pub use ProjectConventions` |
...
- `promethean/.前代产品-worktrees/architect2/366c3e39-rebase1` (HEAD 455d1247, branch `team/architect2/366c3e39-rebase1`)
- `promethean/.前代产品-worktrees/integrations/527f21de-...` (branch `integration/r14-discussion`)
- `promethean/.前代产品-worktrees/integrations/f0d5100a-...` (branch `team/f0d5100a-...`)
- `promethean/.前代产品-worktrees/r10-ao-retry2` (branch `agent-orchestrator/r10-ao-retry2`)
...
源仓 → Desktop (排除清单):
- 排除: `target/` / `dist/` / `build/` / `.tmp-*` / `logs/` / `.openclaw-*.log` / `*.bak` / `*.tmp` / `apeireth-legacy/` (或保留由主人定) / `.前代产品-worktrees/` (父仓用, 源仓里没有) / `.config/` / `.well-known/` (按需) / IDE 文件 (`.vscode/` 等)
