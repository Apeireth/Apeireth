...
- **风险**: 15 untracked 文件被删, R21 续补时需重建 (e.g. apeireth-formal FormalEngine impl 跨 4 backend 需重新设计, 1:1 跟 v0.9.21 skeleton 但要补完整); per fix-cargo-test-workspace §0 D-5, **15 untracked 文件删除决策待 Mavis 拍板**
...
- **决定**: per `1.0-release-test-100-2026-08-06.md` §0 TL;DR:
  ...
  - 0 主动 commit
- **理由**: 14 crate 集成测试原本在 `tests/` 顶层, 不被 workspace 自动 pick up, 搬 sub-workspace 是 1:1 跟 v0.9.21 模式 (per 报告 §1 详)
...
- **理由**: 14 crate 集成测试原本在 `tests/` 顶层, 不被 workspace 自动 pick up, 搬 sub-workspace 是 1:1 跟 v0.9.21模式
