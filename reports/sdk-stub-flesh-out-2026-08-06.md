...
1. **真接层 0 留 R21+ 续**: v0.9.21 livekit-server SDK (Rust crate `livekit-server-sdk` + `tokio-tungstenite` wss://) 0 引. 当前 STUB 6 API 全部 `NotImplemented`. 真接需 1 owner × 1 周 (per lib.rs §9 占位扩展点).
...
1. **真接层 0 留 R21+ 续**: v0.9.21 docker / firecracker / gvisor 客户端 0 引 (`bollard = "0.15"` / `firecracker-rs = "0.5"` / `runsc = "..."` 0 引, per Cargo.toml 注释 ⏳). 当前 STUB 6 API 全部 `NotImplemented`. 真接需 1 owner × 1.5 周 (3 runtime 兼容性差异大).
...
| 块 | 1:1 翻译 v0.9.21 | R21+ 真接路径 | 集成点 | K-1 强校验 |
|---|---------|----------------|--------|------------|
