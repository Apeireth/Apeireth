...
推荐在 OCI archive 上用 `skopeo inspect --raw oci-archive:...` 或 `crane manifest` 对明确引用执行解析，并把原始 JSON 保存到 `logs/v2-alphaon`。若最终使用远端 dry-run staging registry，必须以 digest而不是可变 tag检查，避免检查与发布之间发生 TOCTOU。
