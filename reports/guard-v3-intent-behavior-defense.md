...
| Threshold classifier | `SHADOW_ONLY` | deterministic heuristic; not trained ML |
| ONNX model backend/artifact | `DEFERRED` | no production model or runtime dependency is claimed |
...
The full workspace gates, frontend gates, remote CI, model training, ONNX
export/load, and performance percentile report remain to be run or implemented
and are intentionally not represented as green here.
