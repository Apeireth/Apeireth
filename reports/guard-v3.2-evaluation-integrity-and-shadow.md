...
Dataset manifest: `artifacts/guard-dataseton`.
...
`REAL_SHADOW_EVIDENCE = UNAVAILABLE`: `artifacts/guard-dataset-v3.jsonl` is absent. `shadow_fixture.jsonl` was used only by `--self-test`; it is not production evidence and is not used for readiness claims. No `guard-shadowon` or `review_queue.jsonl` was generated from the fixture.
