...
跳过默认目录: `.git` / `node_modules` / `target` / `dist` / `build` / `_build` / `.venv` / `venv` / `__pycache__` / `.pytest_cache` / `.mypy_cache` / `.cargo` / `.rustup` / `vendor` / `.gradle` / `out` / `_v1_tools_backup` / `_v1260_deploy_*` / `_v1053_pipeline` / `.前代产品-worktrees` / `.tmp*` / `.openclaw-*` / `.idea` / `.vscode`. `--include-build-dirs` 可覆盖.
