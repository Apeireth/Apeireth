...
> 写给任何接手的人（人或 AI）。最后更新：2026-10-11，状态与代码 HEAD 对齐。
> **现状 (2026-10-11)**：前端部分（Tauri 2 + Svelte 5 场景引擎、三模式骨架）仍然有效；
...
> **2026-10-11 追记**：2.0 差距补齐后主人逐批复验收的界面批已全部落地（能力中心三页 /
> 对话栏位常驻控件 / 会话分组 / 窗框融合 / 工具状态翻面，见 §2 末「2026-10-11 批」与
> 台账 `docs/04-internal/live-verification-ledger.md` #66–#75）。
...
- **2026-10-11 批（2.0 差距补齐后的主人逐批复验收批，台账 #66–#75，细节以台账为准）**：
  ...
  - **窗框融合**（主人「不要窗口概念」批）：会话头 `.chat-head` 实底
    `box-shadow 100vmax` 全出血暗带；会话列表栏去浮卡（margin/圆角/描边全去）四边
    锚定窗口；主页「谁找我了」sticky 实底常驻栏顶。栏体 overflow 裁剪不出血，
    右区保留壁纸显影。
  - **工具状态三面翻面**（「永远执行中」修复）：SSE tool-result / 审批 resolve /
    工作台 turn 真值合并三面补 status；历史卡死数据诚实标注「已结束」（后端读模型
    不存工具名，无法逐条回溯——考古结论，勿再试图回填）。
  - 质量门不变：`pnpm check` 0/0 + 测试 15/15 套件；视觉改动 CDP 连拍亲审。
