...
**✅ 落地记录 (2026-10-10, 资深工程师线)**: 头注全读定案 —— 四算法语义 =
betti 认知空洞→追问驱动 / residual 语义新颖度门控 / river spike 传导 / kuramoto
顿悟涌现。**接线 = `cognitive.absorption_insight` 认知体操模块** (实验性):
`AfterTurn` 对本轮消息跑四算法分析 (纯确定性字节直方图特征, 0 LLM/0 IO),
`TurnStart` 注入「【认知体操】」洞察 (只注一次)。旋钮
`APEIRETH_ENABLE_ABSORPTION_INSIGHT=1` 默认关; 测试: 默认关 + 四节可见/同输入
恒同输出/空输入空报告。**0 假装边界**: 浅层确定性特征 (非语义嵌入, 与 morphology
手调启发式同口径), 产出是实验性洞察, **不参与生产语义召回与决策** —— 研究吸收批
的涌现行为试验场。
