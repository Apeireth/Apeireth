...
> **关系文档**（按读序）：本文 → `engineering-log-2026-09-22.md`（30 支 commit 全索引）→ `live-verification-ledger.md` #29/#30（验收真账）→ `frontend/design-preview/spectrai-gap-and-frontend-plan.md` §7（施工结果对照）→ `frontend/companion-desktop/docs/k3-team-kickoff-prompt.md` + `k3-steering-playbook.md`（K3 的施工组织方式，下一批可复用）。
...
- `artifacts/` 里还有一大批 K3 之前的 SpectrAI 装机/窗口调试 ps1/png——历史 scratch，与 K3 无关。
...
8. `main` 是否推送远端 `jimmy`。
9. ~~**全局浅色/深色模式口径**~~ **✅ 已拍板（主人 2026-09-23 当日确认）：Essence 即「浅色模式」并继续做到位**（不另起新主题）；记忆卷宗 Archive 纸面维持「纸面恒纸面」不动。落地时连同梯队 6 主题补全一起做，亮色令牌以 essence 为准。当日已随真机反馈批修掉一批浅色破绽：M/◯ 面板被 scrim 层叠困层糊化、面板浅底对比不足、保存栏不随主题变色。

## 6. 下一梯队（未开工，按 spectrai-gap §6/§7 顺序，等主人指令）
...
| `frontend/design-preview/spectrai-gap-and-frontend-plan.md` | 施工层；§7 = K3 七项结果对照表 |
