...
| tool_bridge.rs | 工具桥: 洋葱门/评审/权限/路径/隔离/spill/post 钩子 | 干活链路 (TP21 fix: 测试代码 `bridge.registry()` 方法不存在 → 字段访问 `bridge.registry` Arc 自动 deref; N17 catalog len 断言 `==9` → `>=9` 因战役 2-5 + N17 双源装配 18+ 件, 详见 backlog X1) |
...
| **9 工具子 crate register.rs (N17/TP2) + catalog.rs (apeireth-tool-registry)** | 工具装配 (§10 装配主链第二环, 铁边界: 不合并 crate/不自写调用/各 crate 提供 register): tool-shell→EnhancedShell / tool-fetch→FetchEngine / tool-browser→EnhancedBrowser / tool-codesearch→CodeIntelligence / tool-image-gen→ImageGenEnhanced / tool-image-process→ImageProcess / tool-search→VSearch / tool-filesystem→EnhancedFileOps / repo-tools→RepoQualityAnalyzer — 每 crate `src/register.rs` (Tool trait 适配 JSON→引擎既有方法 + register/unregister 卸载真清理 0 残留); catalog.rs CapabilityCatalog 只读快照 (from_registry 确定性按名排序 + contains + render_markdown) | ToolBridge::new 统一装配 9 crate + 白名单扩 4 只读工具 (VSearch/CodeIntelligence/RepoQualityAnalyzer/ImageProcess); 执行/网络/写盘类不入白名单走 RiskRule 审批链; 沙盒根 env APEIRETH_TOOL_FS_ROOTS |
