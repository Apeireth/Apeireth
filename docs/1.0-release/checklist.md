...
**0 重复造轮子**: 14 crate 全部 1:1 翻译 v0.9.21, 复用 `apeireth-constraint` (token bucket) + `apeireth-extension` (6 类插件) + `apeireth-keyring` (凭证) 等 LOCKED crate
...
- `LICENSE` (Apache-2.0, 顶部标 @author weibin per v0.9.21 1:1 翻译)
- `NOTICE` (Apeireth 团队 + 致谢: v0.9.21 / Yinta fork / Hermes 工程团队 / code_reviewer / t15-fix-rebase)
