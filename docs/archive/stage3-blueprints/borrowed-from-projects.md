...
| 12 | 6 绫?pluginType 浣滀负椤跺眰 enum 鈥?**鍒ゆ柇閿?* (VCP 鐪熸湁 6 绫讳絾 搂6.1 鎶婂畠褰撳苟鍒楄€岄潪姝ｄ氦) | **淇**: VCP `Plugin.js` + `Plugin/FileOperator/pluginon` (17KB) 鐪熸湁 6 绫?(sync / async / static / service / messagePreprocessor / hybrid),浣?搂6.1 閿欐妸"6 绫诲苟鍒?褰撶粨璁?**鐪熻鍚庢敼涓?5 杞存浜?+ 6 绫讳綔涓?enum 浣嗘瘡绫绘湁 5 杞村睘鎬?* (瑙﹀彂/绛夊緟/椹荤暀/浼犺緭/杈撳嚭) | `research/source/vcptoolbox/Plugin.js` + `research/source/vcptoolbox/Plugin/FileOperator/pluginon` |
