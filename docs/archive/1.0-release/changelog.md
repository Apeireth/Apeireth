...
**改了什么**:
- 新增 5 crate 入 `crates/`:
  - `apeireth-mcp-ssh` (13.8KB / 283 行 lib.rs + 5/5 测试) — 1:1 翻译 v0.9.21 `out/main/mcp/SSHMcpServer.js` (438KB), 8 工具 + 5 认证 + SecretString
  ...
  - `apeireth-workflow` (56.7KB / 1,473 行 lib.rs + 15/15 测试) — 1:1 翻译 `chunks/WorkflowGenerator`, 7 NodeType + DAG + 拓扑排序 + 循环检测
...
- 新增 9 crate 入 `crates/`:
  - **3 估缺核心**:
    - `apeireth-image-prompt` (30.2KB / 817 行 lib.rs + 19/19 测试) — 1:1 翻译 `chunks/ImagePromptLibrary`, 6 类 + sha256 去重 + LRU 1000 + 模板变量替换 + 5 星评分
    - `apeireth-rollback` (38.4KB / 1,040 行 lib.rs + 23/23 测试) — 1:1 翻译 `chunks/RollbackService`, **71GB 4 重防御 hardcode**: TTL 7d + 单影子 100MB + 总 2GB + 3 清理钩子
    - `apeireth-plugin` (32.0KB / 816 行 lib.rs + 20/20 测试) — 1:1 翻译 `chunks/PluginManager` (12KB obf), Marketplace 安装器 (GitHub URL → scan → copy), 0 命中 wasmtime/VM2, 子进程隔离
  - **2 估缺工具**:
    - `apeireth-repo-scan` (24.4KB / 617 行 lib.rs + 5/5 测试) — 1:1 翻译 `chunks/RepoScanAdapter`, 13 Language 枚举 + 11 KEY_FILE_PATTERNS glob + 8 工具
    - `apeireth-repo-analyzer` (34.8KB / 867 行 lib.rs + 17/17 测试) — 1:1 翻译 `chunks/RepoAnalyzer`, 5 TechDebt 枚举 (Todo/Fixme/Hack/Bug/SecurityIssue) + 3 报告格式 (json/markdown/sarif) + m3 防御 3 道
  - **2 基础设施 P0**:
    - `apeireth-keyring` (36.4KB / 972 行 lib.rs + 17/17 测试) — 1:1 翻译 `chunks/keychain-token-store`, 5 重防御 hardcode: PBKDF2 600_000 + AES-256-GCM + 4 Platform + Win CM 真链路 + SecretBytes 脱敏
    - `apeireth-machine-id` (14.0KB / 359 行 lib.rs + 4 平台 424 行 + 26/26 测试) — 1:1 翻译 `chunks/getMachineId-`, 4 平台 fallback chain + 17 hardcode + Windows 11 真跑通 (wmic → registry fallback 拿 MachineGuid)
...
**为什么改**: 蓝图 §2 16 估缺 crate 中 5 P0 落地后, 9 skeleton 是次优先级, 覆盖核心 3 + 工具 2 + 基础设施 P0 2 + SDK stub 2, 1:1 翻译 v0.9.21所有估缺模块。
...
- 6 endpoint 各自 1:1 翻译 v0.9.21 6 tool.js, 复用 apeireth-tools (LOCKED) + apeireth-extension (LOCKED) 6 类插件
...
- claude-code Provider client skeleton: 1:1 翻译 v0.9.21 `claude-code.js`, 复用 apeireth-protocol (LOCKED) + apeireth-constraint (LOCKED)
...
- 1:1 翻译 v0.9.21 `taskTools.js` 估缺 1500 LOC
...
- 1:1 翻译 v0.9.21 `apeireth-sdk.js`, 8 工具 stub + STUB_MODE 编译期守门
...
- 更新 `LICENSE` (Apache-2.0, 顶部标 @author weibin per v0.9.21 1:1 翻译)
...
| 锚 | R20 阶段 1-6 穿透 |
|---|------|
| **S-1** ASI 完整性 | 14 new crate 1:1 翻译 v0.9.21, 0 重设计; WS 8 帧 + 鉴权 5 组件 1:1 蓝图 §2.3 / §2.4 |
