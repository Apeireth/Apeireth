...
> **决策者**: Mavis + 主人 + 架构师
> **作者**: technical_writer
> **性质**: 第十一个 ADR — 记录 Apeireth R19+ 通过 1:1 翻译 前代产品 v0.9.21 `supervisorPrompt.ts` (808 LOC) 实现"团队 leader 给子 agent 的 prompt 构造器"的工程期设计决策, 核心是**新 crate 命名** (A/B/C 三选一) + **`buildAwarenessPrompt` / `buildSupervisorPrompt` + 14 supervisor 工具描述 1:1 翻译** + **不与 `apeireth-supervisor` 命名冲突**。
>
> **依据**: `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §5.2 第 7 行 + §8 决策清单 #3 + §9.2 + ADR 0010 (`apeireth-mcp` 来自 前代产品 翻译) + ADR 0007 (兼容组件层) + ADR 0009 (integration rebase skip 策略) + APEIRETH-CONVENTIONS §9 6 锚穿透。
>
...
| **B** | `apeireth-前代产品-bridge` | 前代产品 血统 | ❌ 跟 6 compat 平台"平台+桥接"风格冲突，且锁死来源命名 |
...
### 前代产品 `supervisorPrompt.ts` 现状（v0.9.21 事实证据）
...
3. 不锁死来源 (不像 `apeireth-前代产品-bridge` 写死 前代产品)
...
- **不翻译**: 前代产品 自己的 TypeScript 类型声明 / 测试代码 (TypeScript-only)
...
- ✅ **14 supervisor 工具的 prompt 1:1 翻译** (跟 前代产品 原文对照可测, prompt 敏感度可量化)
...
- ⚠️ **失去: 跨 prompt 框架兼容** (前代产品 的 Claude 习惯 prompt 不直接兼容 minimax 习惯, 需主人测后迭代)
...
- ✅ 不锁死来源 (未来从 前代产品 演进到其他 prompt 源, 命名不变)
...
### 选项 B: `apeireth-前代产品-bridge` ❌

**优点**:
- ✅ 强调 前代产品 血统, 方便追溯 (1:1 翻译来源清晰)
...
- ❌ "前代产品" 命名锁死来源, 限制未来演进 (如果未来合并 minimax 自研 prompt 源, 命名尴尬)
- ❌ "bridge" 暗示"跨语言/跨进程桥接" (但本 ADR 是 1:1 Rust 翻译, 不跨语言)
- ❌ 跟 `apeireth-pybridge` 职责混淆 (pybridge 是真的跨语言 PyO3 桥, 前代产品-bridge 不是)
...
- 🔴 **前代产品 v0.9.21 supervisorPrompt.ts 808 LOC 翻译完成度未知**（主人 m3 测后才知道 prompt 敏感度）
...
| **S-2 主 17:43** | 6 anchor 实验室 | 1:1 翻译 supervisorPrompt.ts 808 行, 跟 前代产品 原文对照可测, 实验室态度 |
...
- **前置**: [ADR 0007 兼容组件层](0007-compat-components-layer.md) + [ADR 0009 integration rebase skip](0009-integration-rebase-skip-policy.md) + [ADR 0010 apeireth-mcp 来自 前代产品 翻译](0010-mcp-from-前代产品-agentmcpserver.md)
- **蓝图**: `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §5.2 第 7 行 + §8 决策清单 #3 + §9.2 阶段 3 路线
- **Tauri 资产**: `docs/stage4/tauri-assets-from-前代产品-2026-08-05.md`
