# ADR 0012: 前代产品 0.9.21 1:1 翻译 — Apeireth Rust 重写的"对标基础"
...
> **原版 ADR**: [`archive/r20-pre-renumber/0023-前代产品-reverse-engineering.md`](archive/r20-pre-renumber/0023-前代产品-reverse-engineering.md) (v0; v1 本 ADR 引用新编号 0001/0002/0003/0006/0009)

---

## 1. 背景 (Context)

Apeireth Rust 重写 (本仓库) 是 **前代产品 0.9.21** (2024-2025, Apache-2.0) 的 1:1 翻译 + 增量扩展。

**问题**:
- 前代产品 0.9.21 是 TypeScript 实现, 17 个核心模块 + 若干扩展
- 本仓库是 Rust 重写, 1.0 release (v1.0.0) 目标: "行为 / API 跟 前代产品 0.9.21 完全一致, 性能更好, 部署更简"
...
| # | 前代产品 0.9.21 模块 | 本仓库 crate | 状态 | 备注 |
|---|---|---|---|---|
...
### 2.2 R14+ 立体架构 v2 增量 (本仓库独有, 不在 前代产品 0.9.21)
...
- **API 一致**: HTTP API schema 跟 前代产品 0.9.21 100% 兼容 (per [0006-d-01-tool-endpoint-real.md](0006-d-01-tool-endpoint-real.md) D-01)
- **数据兼容**: SQLite 5 表 schema 跟 前代产品 0.9.21 100% 兼容 (per [0009-d-07-sqlite-to-postgres.md](0009-d-07-sqlite-to-postgres.md) D-07)
...
- ✅ **行为一致**: 用户从 前代产品 0.9.21 切 Apeireth-rust 0 学习成本
...
- ⚠️ **生命架构 v4/v4.1** 是 R14+ 增量, 不在 前代产品 0.9.21; 1.0 release 必须重新设计验证

### 3.3 风险

- 前代产品 0.9.21 行为变更 (上游) — 本仓库独立分支, 不跟随, 1:1 锁 0.9.21
...
### A. 推倒重写, 不 1:1
- 优点: 自由设计
- 否决: 失去 前代产品 0.9.21 行为一致优势; 用户升级成本高; 主人 2026-07-30 拍板 "1:1 翻译 + 增量"
...
- 优点: 快 + 简
- 否决: R14+ 立体架构 v2 是 Apeireth 跟 前代产品 0.9.21 最大区别, 砍了等于 "换皮 前代产品"
...
- ✅ **6 哲学锚穿透**: §5 自检
- ✅ **不依赖 NewAPI**: 自建 Rust 重写, 0 依赖 前代产品 0.9.21 上游
...
- 原版 ADR v0: [`archive/r20-pre-renumber/0023-前代产品-reverse-engineering.md`](archive/r20-pre-renumber/0023-前代产品-reverse-engineering.md)
...
**9 估缺 (R20 阶段 1 续, 1:1 翻译 v0.9.21估缺)**:
```
...
| API 一致 | HTTP API schema 跟 前代产品 0.9.21 比对 (per [0006-d-01](0006-d-01-tool-endpoint-real.md) D-01) | 6 工具 100% 兼容 |
| 数据兼容 | SQLite 5 表 schema 跟 前代产品 0.9.21 比对 (per [0009-d-07](0009-d-07-sqlite-to-postgres.md) D-07) | 5 表 100% 兼容 |
