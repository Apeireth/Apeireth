# ADR-0010: apeireth-mcp 来自 前代产品 AgentMCPServer 翻译

```
[Document-Meta]
Document: docs/adr/0010-mcp-from-前代产品-agentmcpserver.md
...
> **决策者**: Mavis + 主人 + 架构师
> **作者**: technical_writer
> **性质**: 第十个 ADR — 记录 `apeireth-mcp` 从 LOCKED 0 代码 → 通过 1:1 翻译 前代产品 v0.9.21 `AgentMCPServer.ts` (893 LOC) 实现的工程期设计决策，附带 14 个 MCP 工具语义 (8 supervisor + 3 worktree + 3 认知) + mid-task bug 3 处修法合并到 `apeireth-mcp::team` 模块的归属决策。
>
> **依据**: `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §5.2 + §8 决策清单 #2 + §9.2 + ADR 0007 (兼容组件层) + ADR 0009 (integration rebase skip 策略) + APEIRETH-CONVENTIONS §9 6 锚穿透。
>
...
| **架构** | 3 transport (http_streamable / sse / stdio) + JSON-RPC 2.0 protocol + `ToolHandler` 桥 | `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §6.2 |
| **缺口** | 服务端 0 业务工具 (only `ToolHandler` trait 表面) | 同上 |
| **LOCKED 历史** | §14.4 crate 候选 + §16.5 对比表都提及，**0 代码** | ADR 0007 §事实证据 |
| **R19+ 填坑目标** | 通过 前代产品 `AgentMCPServer.ts` 翻译补 14 个业务工具 | 蓝图 §5.2 第 6 行 |

### 前代产品 `AgentMCPServer.ts` 现状（v0.9.21 事实证据）
...
**正式确立 "`apeireth-mcp` 来自 前代产品 `AgentMCPServer.ts` 翻译" 路径**，按 5 项硬决策：
...
| **O-2 主 19:33** | 走在前人经验上 | 工具语义 1:1 翻译，可对照 前代产品 v0.9.21 测试 |
...
- **蓝图**: `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §5.2 第 6 行 + §8 决策清单 #2 + §9.2 阶段 1 路线
- **mid-task bug 详**: 蓝图 §4（3 处根因 + 修法表）
- **源码引用**: `AgentMCPServer.ts:457-870`（14 工具定义完整清单）

---

_ADR 0010 草拟 (technical_writer) — `apeireth-mcp` 来自 前代产品 `AgentMCPServer.ts` 翻译路径正式确立, 不修改任何 LOCKED 文档 / 现有架构 / 现有 ADR._
