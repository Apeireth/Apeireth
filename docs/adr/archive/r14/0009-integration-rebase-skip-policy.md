...
- ⚠️ **`team_conflict_skip` 机制需要 前代产品 平台支持**：本 ADR 是设计提议，需平台层实现该 MCP 工具
...
| 阶段 | 任务 | Owner | 依赖 |
|---|---|---|---|
| 阶段 4 | 前代产品 平台层实现 `team_conflict_skip` MCP 工具 | 前代产品 平台 | 本 ADR |
...
- 🔴 **`team_conflict_skip` 当前未实现**：本 ADR 是设计提议，需 前代产品 平台层支持
