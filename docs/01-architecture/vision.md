...
2. **Agent 平台**（v1 era, 86-crate 描述）：85 + 1 desktop crates 三层生态——模块/套件/插件 + Svelte 5/Tauri 2 桌面伙伴 (post-1.0.0 PR #1)，工具管线每一步可校验可审计。**v2 工程重构后 = 18-crate single-workspace (foundation/engine/capabilities/adapters) + legacy/ 完整 v1 归档, 三层模型弃用, 单 plugin 层 trait-based.**
