...
| **蓝图 → reports** | 蓝图 §4 引用 reports/前代产品-architecture §3.2 | grep 相对路径 |
| **reports → 蓝图** | reports/apeireth-crate-api §6 引用 蓝图 §5.3 | grep 蓝图路径 |
| **实施 → 词条** | team-lead-implementation-guide §2 引用 glossary-前代产品 §team-lead 词条 | grep 词条名 |
...
| D-03 | 24 维具体分类名 (continuity / salience / identity / reflection / 4 other) | 前代产品 §7.4 + asi 24 dim |
    ...
    steps:
      ...
      - name: Check R11 baseline 3 值守门 (仅战略/收口文档)
        run: |
          ...
          required_docs=(
            "docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md"
...
| 类别 | 数量 | 文档 | SOP 引用规则 |
|---|---:|---|---|
| **前代产品 架构** | 1 | 前代产品-architecture | 蓝图根源, 引用 §模块号 |
...
**总收口 + 索引**:
- [R19+ 集成收口报告 (24 份文档地图)](file:///) — 本 SOP 的总索引
