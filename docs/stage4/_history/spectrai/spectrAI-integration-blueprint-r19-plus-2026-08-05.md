# 前代产品 → Apeireth 深度集成蓝图 (R19+)

```
[Document-Meta]
Document: docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md
...
> **版本错位诚实标注 (per sub-agent D 前代产品-branch-coverage-audit-2026-08-05.md §3, 2026-08-05 19:01)**:
> 蓝图标题写 "v0.9.21"，但**实际逆向源是 v0.4.6 社区版 main branch** (git HEAD 真实版本)。
> v0.9.21 1.75M LOC 全闭源 (Teams/Workflow/Telegram/Planner 4 大模块 0 逆向)。
> 主人 NSIS 实际是 Yinta fork (new-unpacked/package.json 标 fork.fromVersion: 0.9.21 + author: chuling@local)。
> R20 阶段 1-3 实施时, 应以 v0.4.6 社区版 + Yinta fork 18 万行反编译源为基线, 不以 v0.9.21。
```

> 决策: Mavis 默认 A 方案 apeireth-team-lead, 主人 2026-08-05 13:34 拍板采纳。

> **性质**: R19+ 战略蓝图 = 把 前代产品 v0.9.21 (TypeScript/Electron AI agent 编排, 19 模块 ~25.6K LOC) 的实战模式**深度集成**到 Apeireth v2.0.0-alpha (Rust 41 crate, R19 工程化收尾完成).
...
| 端 | 状态 | 痛点 |
|---|---|---|
| **前代产品 v0.9.21** | TypeScript/Electron 桌面 app, 19 模块, ~25.6K LOC, 5 Provider (Claude/Codex/Gemini/iFlow/OpenCode) 全实装, Supervisor-Member 团队协作已跑通 | 原版卡死 (mid-task bug), minimax m3 48+ context 下 hallucination, mid-task bug 3 处组合 (SessionManagerV2:642 throw + AgentManagerV2:281 catch+success + child session 状态窗口期) |
...
❌ **不**采用 patch-fork 前代产品 (Electron 桌面已验证有 m3 hallucination + mid-task bug 撕裂状态)
✅ **改路线**: 把 前代产品 19 模块**实战模式** → **深度集成**到 Apeireth (TS→Rust 翻译 + Apeireth 41 crate 已有的能力复用)
...
> 前代产品 v0.9.21 = 实战检验过的**工程设备** (5 Provider 抽象 + Supervisor 团队 + Git worktree + MCP 桥), 但有 3 处已知 bug + 1 个 m3 hallucination 风险
...
完整表见 `前代产品-architecture-2026-08-05.md` §2. 摘要 (按集成相关度):
...
| 轨 | 状态 | 集成 | 节省 |
|---|---|---|---|
| **V1 PTY** (SessionManager 674 + AgentManager 1570 + node-pty + HeadlessTerminalBuffer) | ⚠️ deprecated (前代产品 自己标) | 沉淀 T-001 (Tauri 阶段备用) | -30% LOC |
| **V2 SDK** (SessionManagerV2 1231 + AgentManagerV2 963 + Adapter 5 Provider 5564) | ✅ 主路径 | TUI 集成 | +0 |

**砍 V1 理由**: 前代产品 自己在 `sessionHandlers.ts` 头部注释"V1 PTY 路径已弃用"; V2 已全跑通; 集成到 Rust 端不该复活 30% 弃用代码.

### 2.3 分层架构图 (引用 sub-agent 报告)

完整 4 层架构图 (Bootstrap → Core → Adapter → External + IPC Bridge) 见 `前代产品-architecture-2026-08-05.md` §3.1.
...
## §3 前代产品 关键数据流 (引用 sub-agent 报告 5 sequence)

5 个 Mermaid sequence 完整定义见 `前代产品-architecture-2026-08-05.md` §4. 摘要:
...
## §5 前代产品 → Apeireth 集成映射表 (核心交付)

### 5.1 命名空间冲突 (新人接手最大坑)

| 前代产品 概念 | 主人 2026-08-05 提到 | 实际 Apeireth crate | 冲突? | 集成策略 |
|---|---|---|---|---|
| **"api 模块"** | "我记得 Apeireth 就有 api 模块吧" | **`apeireth-api`** (R17 重构, 唯一 LLM 整合点) | ✅ 已确认 | 5 Provider → `apeireth-api` (用户最新决策), 走 4 协议端点 + base_url 配置 |
| **"supervisor"** | 前代产品 高频词 (team lead) | `apeireth-supervisor` (PID 1 进程级 supervisor) | 🔴 强冲突, 含义不同 | 新命名 `apeireth-team-lead`; supervisorPrompt 翻译为 `team_lead.md` |
...
| **"MCP 集成"** | 前代产品 跨进程 MCP | `apeireth-mcp` (LOCKED 0 代码) | ✅ 正好填坑 | 前代产品 AgentMCPServer 翻译为 `apeireth-mcp` 服务端 |
| **"WebSocket 桥"** | 前代产品 AgentBridge (130 LOC) | `apeireth-bus` L4 WebSocket (已有) | 用户决策: **砍**, 改 in-process | ⚠️ 跟 sub-agent 报告"主人 2 决策保留"冲突, 见 §11 风险 + §8 决策清单 #2 |
| **"扩展"** | 前代产品 5 个 Provider Adapter | `apeireth-api` (用户决策) vs `apeireth-extension` (sub-agent 报告) | ⚠️ 双路径 | ⚠️ 见 §8 决策清单 #1, **以 apeireth-api 为主, apeireth-extension 为备** |
| **"Pipeline 5 步"** | 前代产品 工具调用链 | `apeireth-pipeline` (1794 LOC, 已实装) | ✅ 已有, 直接复用 | 前代产品 工具调用逻辑翻译为 `pipeline::step` |
| **"graph / 流程图"** | 前代产品 Worktree 流程 | `apeireth-graph` (565 LOC, 已实装) | ✅ 已有, 直接复用 | Worktree 流程建模为 graph DAG |
| **"tools / 工具"** | 前代产品 toolMapping 5 HashMap | `apeireth-tool-{registry,runtime,approval}` | ✅ 已有, 直接复用 | toolMapping 进 `apeireth-protocol::tool_mapping` |
| **"git worktree"** | 前代产品 GitWorktreeService (746 LOC) | 暂未找到独立 git crate | ❌ 缺 | 翻译为新 `apeireth-git` (1000 LOC, P1) |
| **"storage"** | 前代产品 SQLite + 11 repo | 暂未找到独立 storage crate (apeireth-memory 8/5 是记忆) | ❌ 缺 | 翻译为新 `apeireth-storage` (1300 LOC, P1) |
| **"tracker"** | 前代产品 FileChangeTracker (511 LOC) | 暂未找到 | ❌ 缺 | 沉淀 T-013 |

### 5.2 完整集成映射表 (TUI 集成, P0 → P2)

| 前代产品 模块 | 关键文件 | 对应 Apeireth crate (R19+ 位置) | 集成方式 | 风险点 | 优先级 | 状态 |
|--------------|---------|--------------------------------|---------|-------|--------|------|
...
| **migration (legacy)** | `migration.ts` | **跳过** | 前代产品 旧用户数据迁移, Apeireth 全新安装 | — | — | ⚪ 跳过 |
...
### 6.2 9 个核心 crate 详细 (按 前代产品-architecture §5 集成点)

详细 API 表面见 `apeireth-crate-api-2026-08-05.md` §2. 关键集成点摘要:

| Crate | 关键集成点 |
|---|---|
| `apeireth-council` | ① `LlmAdvisorBackend` 已是真 LLM 入口 (前代产品 多模型路由可直接走); ② `SovereigntyHook` trait 留空, MVP 用 `NoopSovereigntyHook`; ③ `CouncilEvent::OpinionIssued` 事件流 (前代产品 mid-task 显示每个 advisor 意见); ④ `Council::deliberate()` **同步阻塞**, async 环境需 `tokio::task::spawn_blocking` |
...
| `apeireth-mcp` | ① 3 transport 都真实现 (stdio spawn 子进程 / SSE skeleton / HTTP Streamable skeleton); ② `McpServer::from_registry()` 一行代码桥接全部 tool (前代产品 接 MCP 客户端时 server 端零改造); ③ `ToolHandler` 是 `Box<dyn Fn(Value) -> Pin<Box<dyn Future<...>>>>` 灵活; ④ **单飞 (in-flight at a time)**: skeleton 简化, 生产环境要 outstanding 改造; ⑤ MCP 协议版本 hardcode `"2025-03-26"` |
...
| `apeireth-tool-approval` | ① 5 规则按顺序 (Blacklist > Trust > Risk > Frequency > Whitelist), 第一个非 NoMatch 生效; ② `ApprovalHandler` 是外部注册 (前代产品 接 UI 弹窗实现 `set_handler`); ③ 5min 窗口 hardcode; ④ `FrequencyRule` 1min/3 次反刷 (VCP 没有, Apeireth 扩展) |
...
| 缺口 | 位置 | 集成影响 | 修法 (用户决策) |
|---|---|---|---|
| **1. `Pipeline::run_streaming` 是 simulate** | `apeireth-pipeline::Pipeline::run_streaming` line 注释明确 "按 50 字符切块" | 前代产品 真 SSE 推流受阻 | **Mavis 亲自改 1 file** (用户决策 7), 改写为 `stream_to_sender` + `reqwest::Response::bytes_stream`; 改后真 SSE 贯通 |
| **2. `SovereigntyHook` 是 noop** | `apeireth-council::NoopSovereigntyHook` | 前代产品 想"主 AI 守门"无入口 | **MVP Noop** (用户决策 9); R20+ 阶段实现主权仲裁时再 implement `SovereigntyHook` trait |
| **3. `SseTransport` + `HttpStreamableTransport` 是 skeleton** | `apeireth-mcp::transport::*` Week 1 后实装注释 | 前代产品 stdio MCP 翻译不受影响 (MCP 主走 stdio) | 集成时**只用 stdio transport**; SseTransport/HttpStreamable 待 Week 1 后实装; 不影响 P0 阶段 |
...
| **5** | **TUI 风格** | **并排** (用户决策 5) | sub-agent 报告 = "前代产品 24 panel 并排 + Apeireth 9 器官拟人化叠加" (D 方案) | **采纳主人最新指令** = 并排; 不采纳 D 方案的 9 器官拟人化叠加 (避免过度设计); 24 panel 简单并排 (按 4×6 网格) | ✅ 采纳 |
...
| **3. 阶段 4 核心文档 LOCKED** (`6ca80776`) | 我们**新增** `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` 不冲突 |
...
| **TUI 改瘦后 前代产品 工具覆盖可能不完整** | 中 — 某些 前代产品 高级功能缺失 | 中 | ① 前代产品 19 模块中 7 个沉淀到 Tauri (T-001 ~ T-013); ② TUI MVP 跑通核心 (chat + team + worktree); ③ R20+ 阶段补 parser / skill / notify / builtin mcp | **P1** |
| **`apeireth-mcp` LOCKED 0 代码填坑风险** | 中 — 填坑可能改 LOCKED 设想 | 低 | ① 用 `rmcp` crate (官方 Rust MCP SDK) 严格按 MCP 协议; ② 不修改 ADR 0007; ③ 新增 ADR 记录 "apeireth-mcp 来自 前代产品 AgentMCPServer 翻译" | **P0** |
...
| **命名空间冲突: 前代产品 "supervisor" ≠ Apeireth `apeireth-supervisor`** | 高 — 接手者必踩坑 | 高 | ① **新命名 `apeireth-team-lead`** (§5.1); ② supervisorPrompt 1:1 翻译到 `team_lead.md`; ③ 写 ADR 说明命名决策 | **P0** |
...
| **Tauri 团队同步 前代产品 Electron 资产** | Tauri team | 阶段 4 启动前 (R20) | ⚪ 战略级 |
| **R20 收产品时把 前代产品 整合作为 case study** | Mavis → Apeireth leader | 2026 Q4 | ⚪ 战略级 |
| **Apeireth leader 复核本文档** | Apeireth leader | 2026-08-06 | ⚠️ 必复核 |
| **写 ADR: `apeireth-mcp` 来自 前代产品 AgentMCPServer 翻译** | Mavis | 阶段 2 启动前 | 🟡 待写 |
| **写 ADR: `apeireth-team-lead` 新命名决策** | Mavis | 阶段 1 启动前 | 🟡 待写 |
| **更新 GLOSSARY.md** 加 前代产品 / 前代产品 / supervisor / team-lead 词条 | Mavis | 阶段 1 启动前 | 🟡 待写 |
| **更新 APEIRETH-CONVENTIONS.md** §5 报告路径加 `前代产品-integration-blueprint` 模板 | Mavis | 阶段 1 启动前 | 🟡 待写 |
...
| 报告 | 路径 | 大小 | 章节 | 核心 |
|---|---|---|---|---|
| **前代产品-architecture** | `` | 61.2 KB | §0-§11 + 附录 A (34 文件清单) | 19 模块架构图 + 5 sequence + 集成映射表 (§5) + mid-task bug 根因 (§6) + 主人 5 决策 (§8) + TUI 风格推荐 (§9) + 11 周路线 (§5.3) |
| **apeireth-crate-api** | `` | 45.4 KB | §0-§5 + 关键发现 | 9 crate API surface 详细 (council/agent/graph/mcp/pipeline/tool-runtime/tool-registry/tool-approval/protocol/supervisor) + 跨 crate 集成现状 (§3) + 真实 LLM 集成缺口清单 (§4) + 推荐集成顺序 (§5) |
| **apeireth-platform-modules** | `` | 26.2 KB | §0-§8 | **apeireth-api 是 LLM 整合点** (R17 重构, 唯一) + 41 crate 总览 (§1) + `apeireth-api` 详细 (§2) + compat/平台/桥接映射 (§3) + HTTP client + 5 Provider 映射 (§4-§5) + 跨模块集成现状 (§6) + 关键发现 (§7) + 需 Mavis 拍板 (§8) |
...
## 附录 B: 关键源码引用 (前代产品, file:line 完整清单)
