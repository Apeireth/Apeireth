# v0.9.21 vs Yinta fork vs v0.4.6 社区版 — 完整差异审计 (R20 阶段 1-3 实施基线)
...
Author:      Mavis (per sub-agent D 前代产品-branch-coverage-audit + sub-agent E yinta-fork-audit 整合)

> **性质**: 3 个 前代产品 版本完整差异审计, 决定 R20 阶段 1-3 实施基线. **不实施**, **不 commit 源码**, 0 改 crates/apeireth-*/src/.

> **依据** (6 份必读):
> - `` (920 行, 11 章, sub-agent 1 architect)
> - `` (572 行, sub-agent D)
> - `` (504 行, sub-agent E)
> - `` (613 行, sub-agent A)
> - `` (644 行, sub-agent B)
...
| 维度 | v0.4.6 社区版 | v0.9.21 (估) | Yinta fork 0.1.0 (实查) |
|------|--------------|---------------------|------------------------|
| **来源** | wei9966/前代产品 GitHub main branch | wei9966 团队商业版 (闭源 NSIS) | chuling@local fork 自 v0.9.21 + paid tier bypass |
...
| **商业版访问** | 自由下载 | 付费 + NSIS 闭源 | 主人 NSIS 在 `安装包 估 |
...
1. **NSIS 定位**:
   - 主人 v0.9.21 NSIS 在 `` (估)
   - 备份: copy 到 `前代产品-Commercial-0.9.21.exe.bak`
2. **安装包分析** (per Yinta fork 解包经验):
   ```bash
   7z x 安装包 -o前代产品-Commercial-0.9.21/
   ```
...
### §5.2 安装包分析后基线 (1-2 周后, 推荐)

| 阶段 | 实施基线 | 估时 |
|------|----------|-----:|
| 阶段 1 准备 | Yinta fork + 原版 安装包分析 1.3M LOC + 8 闭源模块 | 8h (重做) |
...
- 估时**同** (53h) — 安装包分析前估缺 8 闭源 → 必须 1:1 设计; 安装包分析后直接 1:1 翻译
- 估缺**不同** (8 闭源估缺 → 0 估缺) — 安装包分析后 R21 商业化路径清 (Discord 冷启动 / TaskKanban / Telegram Bot / SuggestionEngine 都有)
- 估时**同** 但估缺**少** → 长期 ROI 安装包分析高
...
| 已有报告 | 引用位置 | 不重复内容 |
|----------|---------|----------|
| `前代产品-architecture-2026-08-05.md` | §1 §5 估缺 + §2 8 闭源 | 不重复 §2 19 模块总览 (引用即可) |
| `前代产品-branch-coverage-audit-2026-08-05.md` | §1 + §2 8 闭源 (per D §3 估) | 不重复 §4 21 项假盲点 (引用即可) |
...
|---|---|
| 路径 | `` |
| 3 版本对比 | v0.4.6 社区版 (26K TS) / v0.9.21 (1.75M 估) / Yinta fork 0.1.0 (446K 业务) |
...
| 合计 | 107h (1 工程师 2-3 周) |
| 商业版访问 3 选项 | A 重买 (❌) / B 团队成员 (❌) / C 安装包分析 1-2 周 (✅) |
...
- sub-agent 1 architect 写的 `前代产品-architecture-2026-08-05.md` 11 章 (920 行)
...
- sub-agent D 写的 `前代产品-branch-coverage-audit-2026-08-05.md` (572 行, 21 项假盲点)
...
**S-2 实事求是登记**:
1. 本报告纯审计, 不写代码, 不 git commit (产出物在 前代产品 工作树)
