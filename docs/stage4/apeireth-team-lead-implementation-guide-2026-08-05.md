...
>
> **依据**: `docs/adr/0011-apeireth-team-lead-supervisor-prompt-translation.md` §决策 1-5 + `docs/adr/0012-team-lead-council-collaboration.md` §决策 1-4 + `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §5.2 第 7 行 + `docs/stage4/glossary-前代产品-additions-2026-08-05.md` 词条 2/3/4。
>
> **约束**:
...
edition = "2024"
description = "R19+ 团队 leader 角色 - 1:1 翻译 前代产品 supervisorPrompt.ts (970 LOC)"
license = "Apache-2.0 OR MIT"
repository = "https://github.com/apeireth/apeireth"
keywords = ["apeireth", "team-lead", "agent", "prompt", "前代产品"]
...
/// 1:1 翻译 前代产品 supervisorPrompt.ts:buildSupervisorPrompt
...
/// 1:1 翻译 前代产品 supervisorPrompt.ts:buildAwarenessPrompt
...
| 维度 | 前代产品 (TypeScript) | Rust (1:1 翻译) | 备注 |
|---|---|---|---|
...
| 前代产品 函数 (supervisorPrompt.ts) | Rust 函数 (apeireth-team-lead) | LOC | 关键点 |
|---|---|---:|---|
...
| `injectAwarenessPrompt(workDir: string): string` | `inject_awareness_prompt(work_dir: &Path) -> String` | 30 | 文件读写 (写到 `前代产品-worktree.md` 同级) |
...
| `injectFileOpsRule(workDir): void` | `inject_file_ops_rule(work_dir: &Path) -> Result<(), Error>` | 20 | 注入到 .前代产品/ 目录 |
...
> 前代产品 实际工具名 (supervisorPrompt.ts 内 14 个 `**tool_name**(...)` 块) → Rust 公开 API 命名 (用户习惯简写)。

| # | Rust 工具名 | 前代产品 原名 | 关键参数 | prompt 描述骨架 (估 30-50 字) |
|---:|---|---|---|---|
...
**翻译约束** (rust-coder 必读):
- ✅ **严格 1:1 翻译**: 前代产品 原文措辞保留 (含中文 markdown 段落)
- ✅ **保留所有参数签名**: (agentId, timeout?) 这种可选参数标注
- ✅ **保留 few-shot examples**: 前代产品 原文的 3-5 行例子完整保留
...
| 来源 | LOC | 备注 |
|---|---:|---|
| 前代产品 原文 (`supervisorPrompt.ts`) | **970** | 实际 `wc -l` (含注释 + 空行) |
...
- 修 前代产品 mid-task bug 3 处不能掉 baseline
...
| `supervisor_prompt_1to1_translation` | 818 行 markdown 跟 前代产品 原文 diff = 0 (主 S-2 实验室) |
...
### 6.5 前代产品 原文 1:1 翻译验证 (主 S-2 17:43 实验室)

> **强验证**: `build_supervisor_prompt` 输出跟 `supervisorPrompt.ts:buildSupervisorPrompt` 输出**逐字符对比**。

**做法** (实施时):
1. 用 `supervisorPrompt.ts` 在 Node.js 跑一次, 输出 818 行 markdown 到 `tests/fixtures/前代产品_supervisor_prompt.txt`
...
| **S-2** | 17:43 | 6 anchor 实验室 — 1:1 翻译 supervisorPrompt.ts 970 行, 跟 前代产品 原文对照可测 (§6.5 逐字符 diff), 实验室态度 |
...
| `docs/adr/0010-mcp-from-前代产品-agentmcpserver.md` | §决策 (apeireth-mcp 翻译路径) | 14 工具 trait 来源 |
| `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` | §5.2 第 7 行 + §8 决策清单 #3 + §9.2 阶段 3 路线 | 蓝图主文档 |
| `docs/stage4/glossary-前代产品-additions-2026-08-05.md` | 词条 2 (team-lead) + 词条 3 (council) + 词条 4 (supervisor) | 4 分类清晰 |
| `docs/stage4/tauri-assets-from-前代产品-2026-08-05.md` | §3 13 项资产清单 | 知道哪些给 Tauri 不用做 |
...
| GLOSSARY.md 8 词条合并 | glossary-前代产品-additions §拍板记录 | R19 阶段 1 启动前 |

### 11.4 源码引用 (1:1 翻译来源)

| 路径 | 用途 |
|---|---|
| `` | 970 LOC 1:1 翻译主源 (含 818 行 markdown) |
