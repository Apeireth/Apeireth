...
> **性质**: 纯设计+代码骨架交付. **不改任何 crate 源码** (含 `crates/apeireth-*/src/` LOCKED 24 crate). 产出物全部在 `前代产品/reports/前代产品-r19plus-v2/`.
>
> **5 份必读上下文** (按主人 2026-08-05 19:01 拍板):
> 1. `` — §1 顶层摘要 + §6 mid-task 根因 + §7.1 m3 风险 (920 行)
> 2. `` — 4 ZST adapter (49.3 KB)
> 3. `` — 10 crate API surface (44.3 KB)
> 4. 前代产品 源: `` (5 Provider adapter, 只读)
> 5. Apeireth 源: `.openclaw\workspace\promethean\Apeireth-rust\crates\apeireth-protocol\src\` (Hermes LOCKED, 只读)
>
> **配套增量引用** (不重复内容, 引用+增量):
> - `前代产品/reports/apeireth-mcp-14-tool-analysis-2026-08-05.md` §2 14 工具白名单 (8 supervisor + 3 worktree + 3 认知)
> - `前代产品/reports/apeireth-asi-24dim-api-2026-08-05.md` §3.1 V05_DIMENSION_NAMES (LOCKED) + §1.2 D-03 第 25 维建议
...
| 8. 8 项 LOCKED (24 crate src/ 不改) | ✅ 0 触碰 `crates/apeireth-*/src/`, 全部在 `前代产品/reports/前代产品-r19plus-v2/` |

### 0.3 报告结构 (5 章 + 1 自检 + 1 边界)

| 章 | 标题 | 关键交付 |
|---|------|---------|
| §1 | 触发模式分类 (4 类) | m3 hallucination 4 类模式 + 前代产品 现状 + m3 vs Claude/Gemini 对比 |
...
> 主人 16:40 战报 + 18:49 强调的 4 个已知模式, 1.1~1.4 逐个拆解. 每类 3 段: 模式说明 / 前代产品 现状 / m3 vs Claude/Gemini 对比.
...
**前代产品 现状** (per `前代产品-architecture-2026-08-05.md` §6.1~§6.4): 主人在 §6.4 已给 3 处修法 (改 throw 为 return Result / sendToAgent await + 检查 child session 状态 / 同步 child 状态到 agent). **但修法在 TS 端, R20 阶段要把 前代产品 翻译到 Rust** (`apeireth-session` + `apeireth-team-lead`, 估 1500-2000 LOC) — 翻译时 **必须不重蹈覆辙**, Rust 端用 `tokio::sync::watch` 跟踪 status + `Result<T, SessionError>` 统一返回.
...
**前代产品 现状** (per `前代产品-architecture-2026-08-05.md` §7.1 第 9 行 m3 minimax API 风险): "minimax 的 `path` 错误要在 validation 层拦截". **当前 validation 层 = 0**. 前代产品 14 工具 schema 在 MCPConfigGenerator 生成, **不在 Rust 端校验** — 走 AgentMCPServer 的 JSON-RPC 解析. AgentMCPServer 是 stdio MCP server, schema 校验依赖 mcp 客户端, mcp 客户端依赖 m3 传对. **链路 0 防御**.
...
8. **任一 ack 超时** → `Result::Err(DualAckTimeout { stage: "received" | "processed", ms })` — **不 `.catch` 吞错, 不 `success: true` 骗父** (per `前代产品-architecture` §6.4 修法 2 关键)

**跟 m3 哪类对应**: §1.2 mid-task bug 3 处组合吞错 — 双层 ack 让 `.catch` 没机会吞错 (父直接看 ack 状态, 不信 success 字段).

### §2.3 m3 特定 context 48+ 监控

**借鉴**: 主人在 `前代产品-architecture` §7.1 P0 缓解第①条 "限制单次 context ≤ 32 messages" 是**软的**; 本报告 §2.3 把它变**硬** + 加 schema 二次校验 + 自动截断长 nested JSON.
...
> 3 fixture 全在 `前代产品-source/tests/fixtures/m3_hallucination/` (TUI 测试用, 不污染 Apeireth LOCKED tests), 实施时由 qa_engineer 写. **设计参考**, 不写实际代码.
...
- 期望:
  ...
  - **不**返回 `success: true` (对比 前代产品 `AgentManagerV2:285` 反例)
  - 父进程收到 Err 后能决定: 重试 / 放弃 / 提示用户 (per `前代产品-architecture` §6.4 修法 2 原则)
- 断言: 0 `.catch(console.error)` 吞错; 0 `return { success: true }` 骗父
- 借鉴: `前代产品-architecture-2026-08-05.md` §6.4 修法 2 (`agent/AgentManagerV2.ts:269-286`)

### §4.3 `test_m3_fabricated_tool_rejected` — 模拟 m3 虚构 前代产品_send_to_agent_v2

**目标**: 验证 §2.4 14 工具白名单能拦下 m3 虚构工具

**fixture 设计**:
- 输入: 模拟 minimax m3 在 48+ context 后虚构
  - `tool_use.name = "前代产品_send_to_agent_v2"` (实际白名单 = `send_to_agent`)
  - `tool_use.input = {"agent_id": "agent-xxx", "message": "..."}`
- 期望:
  - §2.4 `is_whitelisted("前代产品_send_to_agent_v2")` 返 `false`
  - §3.1 `ToolSchemaValidator::validate_tool_call` 在 §2.4 拦截这一步返 `Err(PipelineError::UnknownTool { name: "前代产品_send_to_agent_v2", allowed: WHITELIST.to_vec() })`
...
> R20 阶段 1-5 是主人 19:01 决策的下一阶段路线 (per `前代产品-architecture-2026-08-05.md` §5.3 R1-R6 11 周 + 主人 "R19+ 蓝图缺" 决策). 5 道防御在每一阶段的嵌入点:
...
| 8. 24 crate `src/` 不改 | ✅ 0 触碰 | 0 触碰 `crates/apeireth-*/src/`, 全部在 `前代产品/reports/前代产品-r19plus-v2/` |

### §6.2 6 哲学 anchor 6/6 穿透自检

| Anchor | 穿透位置 | 严守动作 |
|--------|---------|---------|
| **S-1 北极星** = 前代产品 → Apeireth 集成目标 | §0.1 决策 1 (R20 阶段 1-5 集成) + §5 全章 | 5 道防御 = 集成目标的**质量守门**, 不偏离主人 R19+ 终极目标 (TUI → Tauri, 主人 user memory #8) |
...
| **O-4 任何人都能接手** = 5 道防御清晰可读 | §2 5 道 (编号 2.1~2.5) + §3 4 snippet (编号 3.1~3.4) + §4 3 fixture (编号 4.1~4.3) + §5 5 阶段 (编号 5.1~5.5) | 全文编号清晰, 每章 ≤200 行, 接手者按编号顺序读不迷路. 配套引用 `前代产品-architecture` §6.4 + `apeireth-mcp-14-tool-analysis` §3 + `apeireth-asi-24dim-api` §1.2 — 3 份文档互引, 任何接手者搜文件名即可调出 |
...
| 已有报告 | 引用位置 | 不重复内容 |
|----------|---------|----------|
| `前代产品-architecture-2026-08-05.md` | §1.2 (mid-task 3 处) + §1.3 (5 Provider) + §1.4 (m3 风险) + §5 (R20 阶段 1-5) | 不重复 §6.4 修法细节 (引用即可), 不重复 §5 映射表 (引用即可) |
...
| `前代产品-source/src/main/adapter/` 5 Provider | §1.1 (V2 Adapter 架构) + §1.3 (5 Provider 注入机制) | 0 触碰, 仅作为现状引用 |
...
|---|---|
| 路径 | `` |
| 触发模式 | 4 类 (§1.1~§1.4, 每类说明+前代产品 现状+m3 vs Claude/Gemini 对比) |
...
**结论**: Yinta fork = v0.9.21 + paid tier bypass, **完全无 m3 集成**。主人 19:01 拍板"全补"确认 fork 0 翻译 m3。
...
### §7.3 fork = v0.9.21 + paid tier bypass (背景)

- fork 实际版本 0.1.0 (per fork `package.json`), fork 自 v0.9.21, 作者 chuling@local, fork 时间 2026-08-03
- fork = v0.9.21 + paid tier bypass (`getEffectivePlan` 永远 `enterprise`, 改动只在 `out/renderer/assets/index`)
