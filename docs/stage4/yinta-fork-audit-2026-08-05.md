...
> **性质**: **审计报告**, 0 改 Apeireth 任何源码, 0 git add/commit. 产出物只写到 `前代产品-r19plus-v2/`.
>
> **核心结论 (1 段)**: 主人 2026-08-03 NSIS 装的 `安装包 (1).exe` (438 MB, 2026-08-01) **不是 wei9966 原版 前代产品 0.9.21**, 而是 `chuling@local` 的 **Yinta fork** (`new-unpacked/package.json` `fork.fromVersion: 0.9.21`, `author: chuling <chuling@local>`). Yinta fork 实际就是 **v0.9.21 + paid tier bypass** (`getEffectivePlan` 永远返 `enterprise`), 业务代码 446,652 LOC (out/ 171 文件, 排除 node_modules), 跟 D-agent 估的 1.75M LOC 商业版仍有 **约 4 倍 gap** (估缺 Teams/Workflow/Telegram/Planner 闭源 chunks). **fork 内 0 处 minimax m3 集成** (grep `minimax|MiniMax|hallucination|minimax-m3` 全 0 命中), 主人 m3 防御必须**在 Rust 端全新设计**, 不能从 fork 翻译.
...
| 项 | 值 |
|---|---|
| **目标** | 立即扫主人 Yinta fork `new-unpacked/`, 跟 v0.4.6 社区版对比 + 跟 v0.9.21对比 + 看 5 商业版闭源模块是否覆盖 |
| **硬约束** | ❌ 不 git add / commit ❌ 不改 crates/apeireth-*/src/ ✅ 产出物写到 `前代产品-r19plus-v2/` |
| **真实读** | 读 `new-unpacked/package.json` 全文 (86 行) + 算 fork .js 行数 (PowerShell `Measure-Object -Line`) + 列顶层目录 + 关键 bundle + 关键 grep 验证 |
| **6 份上下文** | ① D 报告 `前代产品-branch-coverage-audit-2026-08-05.md` ② 架构 `前代产品-architecture-2026-08-05.md` ③ 集成蓝图 `docs\stage4\前代产品-integration-blueprint-r19-plus-2026-08-05.md` ④ 社区源 `前代产品-source/` ⑤ Yinta fork `new-unpacked/` ⑥ 4 份新蓝图 (m3 / 5-provider / supervisor-prompt / branch-coverage) |
...
| 社区 `前代产品-source/src/main` 全 .ts | **84 文件 / 26,640 lines** |
...
| `description` | "Yinta - Multi Claude Code Session Orchestration Platform (forked from 前代产品)" |
...
| **`fork.fromAuthor`** | **`weibin <bin.wei@steriguard.cn>`** (原版 前代产品 作者) |
...
>   - UI 字符串残留: `window.前代产品.xxx` 734 处 / `前代产品` 95 处 / `前代产品` storage key 67 处 (15.7MB bundle 里)
...
> - MIT License
> - Copyright (c) 2026 weibin (前代产品 original author)
> - Copyright (c) 2026 chuling (Yinta fork modifications)
> - "本项目 (Yinta) 基于 前代产品 v0.9.21 修改而来"

> **CHANGELOG.md 关键事实** (per `new-unpacked/CHANGELOG.md`, GBK 编码损坏, 仅能读 ASCII 部分):
> - 0.1.0 (2026-08-03) Initial Fork
> - 修改了什么: package.json (name/version/author/fork) + `out/renderer/assets/index` (getEffectivePlan + EDITION_DISPLAY_NAME)
> - 不修改什么: `out/main/index.js` (174KB 全 hex) / UI 字符串残留 / 70+ 依赖

### 1.3 实跑算 fork 行数 (vs 社区版 v0.4.6 + 估 v0.9.21)

| 维度 | 社区 v0.4.6 (git HEAD) | Yinta fork `new-unpacked/` | Yinta fork `out/` 业务代码 | 估 v0.9.21 |
|---|---|---|---|---|
| **来源** | `前代产品-source/` (git) | `new-unpacked/` (安装包分析) | `new-unpacked/out/` (业务代码) | wei9966 原版 (估 1.75M LOC per D-agent §1.4) |
...
| **关键文档** | 2 commit / 0 tag | 0 commit (安装包分析) | — | 0 commit (NSIS) |
| **Git 仓库** | wei9966/前代产品 main | 不可用 (NSIS 拆产物) | 不可用 | 不可用 |
...
2. **fork out/ 业务代码 446,652 LOC ≈ 社区版 26,640 LOC 的 17 倍** — fork 显著大于社区版, 包含了 main 分支没有的 v0.9.21 chunks
...
│   │       ├── index                170.1 KB
│   │       ├── index                10.4 KB
│   │       ├── sdk                  920.9 KB / 7 lines ⚠️ 核心 SDK bundle
│   │       ├── stdio                20.3 KB
│   │       ├── stdio                311.9 KB / 7 lines
│   │       ├── taskTools            313.4 KB ⚠️ 任务工具
│   │       ├── tree-sitter-Fukzi_5-.js          631.5 KB ⚠️ 语法解析
│   │       ├── tree-sitter-bash     2,829.0 KB ⚠️ 最大单文件
│   │       ├── WorkflowGenerator-BQCQ_KQx.js   63.7 KB ⚠️ 关键: 工作流生成器!
│   │       ├── PluginManager        11.9 KB
│   │       ├── GeminiAuthHelper     13.0 KB
│   │       ├── ImagePromptLibrary   36.0 KB
│   │       ├── RollbackService      22.6 KB
│   │       ├── compatibility        11,671.7 KB / 1 line ⚠️ 最大兼容层
│   │       ├── keychain-token-storage      11.7 KB
│   │       ├── multipart-parser            7.4 KB
│   │       ├── RepoAnalyzer                5.9 KB
│   │       ├── RepoScanAdapter             6.9 KB
│   │       ├── getMachineId- (7 文件, 每 4-5 KB)
│   │       ├── execAsync (2 文件, 每 1-2 KB)
│   │       ├── InstallErrorCodes           2.7 KB
│   │       └── ScanResult                  1.6 KB
│   ├── preload/                                 渲染桥接
...
|---|---|---|---|---|
| 1 | `index` | `out/renderer/assets/` | **15.03 MB** | ⚠️ 渲染层核心 React bundle, 含 getEffectivePlan / EDITION_DISPLAY_NAME (per `tools/patch-yinta.js` 脚本) |
| 2 | `compatibility` | `` | **11.67 MB** | ⚠️ 兼容性层 (估 polyfill / node-api shim) |
| 3 | `tree-sitter-bash` | `` | **2.83 MB** | ⚠️ bash 语法解析 (file diff 用) |
| 4 | `sdk` | `` | **920.9 KB** | ⚠️ SDK 主 bundle (估 ajv 校验 + provider 集成) |
| 5 | `tree-sitter-Fukzi_5-.js` | `` | **631.5 KB** | ⚠️ tree-sitter 主框架 |
...
| 3 | `taskTools` | **313.4 KB** | ⚠️ 任务工具集 (worktree / kanban?) |
| 4 | `WorkflowGenerator-BQCQ_KQx.js` | **63.7 KB** | ⚠️ 关键: 工作流生成器 (per D-agent §3.1 估有 Teams/Workflow 闭源) |
| 5 | `index` | **170.1 KB** | ⚠️ 通用 chunk (估 adapter / 业务核心) |
...
| # | 社区 v0.4.6 模块 (per `前代产品-architecture-2026-08-05.md` §2) | fork 实际 | fork 替代实现 |
|---|---|---|---|
...
| 16 | task (1 文件 / 123 LOC, TaskSessionCoordinator) | ✅ `taskTools` 313 KB | bundle 化 |
...
| mid-task bug 根因 (per `前代产品-architecture-2026-08-05.md` §6.4) | fork 实际状态 | R20 阶段翻译策略 |
|---|---|---|
...
| **grep `m3` in fork `out/`** | 🟡 18 文件命中, **但全是 Monaco editor `m3` tokenizer** (per 文件路径 `./renderer/monaco/vs/m3` 等) | **没有 1 处 LLM MiniMax m3** |
...
## §4 Yinta fork 跟原 v0.9.21对比

### 4.1 行数对比 (核心数字)

| 维度 | Yinta fork (本报告 §1.3 实跑) | 估 v0.9.21原版 (D-agent §1.4) | 差距 |
|---|---|---|---|
...
| **NSIS installer 大小** | 438 MB (主人的 `安装包 (1).exe`) | 估 450-500 MB (商业版原版可能稍大) | 估缺 5-10% (估缺闭源 chunks) |
| **应用包 大小** | 697 MB (per D-agent §2.1) | 估 800+ MB | 估缺 15-20% |
...
| `TeamBus` | 0 | 🔴 fork 缺 |
| `TaskKanban` | 0 | 🔴 fork 缺 (虽然有 `taskTools` 313 KB, 但名字不同) |
...
| file diff + tree-sitter | `tree-sitter-Fukzi_5-.js` 631.5 KB + `tree-sitter-bash` 2.83 MB |

### 4.4 fork 跟商业版 NSIS 是否同一份?

| 维度 | Yinta fork NSIS (per `安装包 (1).exe`) | wei9966 原版 v0.9.21 NSIS (估) |
|---|---|---|
| 路径 | ` (1).exe` 438 MB (2026-08-01) | 估 wei9966 官网下载 (估 450-500 MB) |
| 内部 asar 内容 | Yinta (chuling fork) | 原版 前代产品 (估) |
| 商业版 plan | 永远 "enterprise" (patched) | 6 tier (anonymous/free/pro/ultra/team/enterprise) per `getEffectivePlan` |
| 品牌 | "Yinta" (patched) | "前代产品 Pro" |

**关键**: **主人装的不是 wei9966 原版, 是 chuling Yinta fork**. D-agent §3.3 第 3 项 "主人 reverse engineer 产物来自 Yinta fork" 已识别这一点. **要拿 v0.9.21原版, 必须从 wei9966 官网 (前代产品dev.cloud) 重新下载**.

---

## §5 v0.9.21访问策略 (3 选项 + 1 推荐)

### 5.1 选项 A: 主人直接去 wei9966 官网重买 / 重下

| 维度 | 详情 |
|---|---|
| **行动** | 主人登 前代产品dev.cloud, 买 enterprise tier (估 ¥999-4999/年), 拿 v0.9.21 (或更新) NSIS installer, 走原版付费墙 |
...
| **R20 影响** | 拿全 v0.9.21, 蓝图可改 22 工具 → 估 30+ 工具, R20 阶段 1-3 可用 fork + 商业版双源 |
...
| 维度 | 详情 |
|---|---|
| **行动** | 主人若是 wei9966 团队成员, 内部 git 拿 v0.9.21 .ts 源码 |
| **优点** | 100% 源码, 不需反编译 |
| **缺点** | **主人不是 wei9966 团队成员** (per user memory §"项目背景", 主人是研究生做学术研究项目) |
| **R20 影响** | 拿全 v0.9.21, R20 阶段 1-3 可 1:1 翻译 |
| **推荐度** | ❌ (主人不是 wei9966 团队) |

### 5.3 选项 C: 主人从 v0.9.21 NSIS 提取反编译源码

| 维度 | 详情 |
|---|---|
| **行动** | 主人从 wei9966 官网 / 朋友 / GitHub 镜像拿 wei9966 原版 v0.9.21 NSIS installer, 走 `安装包 (1).exe` 同样的 7z + asar 解包流程, 产出 `new-unpacked-commercial/`, 跟 Yinta fork 对比 |
| **优点** | 拿到原版, 不付钱 (如果是朋友/镜像) |
| **缺点** | 商业版 hex 化比 Yinta fork 严重 (估 +30% 闭源混淆), 1:1 翻译估 200+ 工时 |
| **估时** | 1-2 天 (解包) + 1-2 周 (反编译 + diff 跟 fork 对比) |
| **R20 影响** | 拿全 v0.9.21, fork 跟商业版 diff 1:1 对比, 估能补 800K+ LOC 闭源 chunks |
...
**立即可做** (per §7):
1. 主人去 wei9966 官网 (https://github.com/wei9966/前代产品) 找 NSIS 下载链接 (或朋友拿原版 installer)
2. 走 `安装包 (1).exe` → `7zr.exe x` + `npx asar extract` 解包, 产出 `new-unpacked-commercial/`
...
| 22. 4 file (前代产品_edit/write/create/delete_file) | ✅ | 同上 |
...
| **GeminiHeadlessAdapter (5 entries)** | ✅ (hex 化) | `@google/gemini-cli` 0.33.1 + `gemini-cli-core` 0.33.1 + `GeminiAuthHelper` 13.0 KB |
...
| `EDITION_DISPLAY_NAME` | "Yinta" (was "前代产品 Pro") | 显示名替换 |
...
| `productName` (electron-builder) | "Yinta" (was "前代产品") | 重打包时设 |
| `window.前代产品.xxx` IPC namespace | 734 处残留 | 视觉上仍显示 "前代产品", 功能不影响 |
...
| 步骤 | 行动 | 估时 | 报告 |
|---|---|---|---|
| **1.1 拿 v0.9.21原版 NSIS** | 主人去 wei9966 官网或朋友拿, 走 `安装包 (1).exe` 同款 7z + asar 解包, 产出 `new-unpacked-commercial/` | 1-2 天 | (per §5.4 选项 C) |
...
| **1.3 重写 4 份蓝图** | ① 5-provider-tool-mapping: 加 1 个 CopilotProvider (6 Provider) ② supervisor-prompt-818: 改 "前代产品" → "apeireth" (保留 Claude Code) ③ m3-hallucination-defense: 加 5 道防御 Rust 端 hardcode ④ 前代产品-branch-coverage: 加 Yinta fork 跟商业版 diff 章节 | 3-5 天 | 4 份蓝图 v2 |
...
| 步骤 | 行动 | 估时 |
|---|---|---|
| **3.1 蓝图重写 5 份** | ① 5-provider-tool-mapping v2 (6 Provider + m3) ② supervisor-prompt-818 v2 (rename + 22 工具) ③ m3-hallucination-defense v2 (实装反馈) ④ 前代产品-branch-coverage v2 (加 fork diff) ⑤ 新: 商业版 vs fork 1:1 diff 文档 | 2-3 天 |
...
| `supervisor-prompt-818-summary-2026-08-05.md` (52 KB) | 7 段 818 行 | 加 §7 "Yinta fork paid tier 旁路, 1:1 翻译时改 '前代产品' → 'apeireth'" (per §6.4) |
| `前代产品-branch-coverage-audit-2026-08-05.md` (42 KB) | 21 项假盲点 | 加 §8 "Yinta fork = v0.9.21 + paid tier bypass, 446K LOC vs 1.75M 估缺 75%" (per §4.1) |
...
| 3 | 不改 docs/stage[1-6]/ (Apeireth LOCKED) | ❌ **0 触犯** | 本审计**0 读 0 写** Apeireth docs 目录 (只引用 `docs\stage4\前代产品-integration-blueprint-r19-plus-2026-08-05.md` 路径) |
...
| 7 | 产出物只写到 `前代产品-r19plus-v2/` | ✅ **遵守** | 本审计**只写** 1 个文件 `yinta-fork-audit-2026-08-05.md` 到 ``, 不污染其他目录 |
...
|---|---|---|
| **S-1** | **北极星 = 扫除盲点** | ✅ §3 (fork vs v0.4.6) + §4 (fork vs v0.9.21) + §6 (fork vs Apeireth 集成点) 8 节, fork 全面扫除 (446K LOC + 5 Provider + 4 MCP + paid tier + m3 缺) |
...
| 维度 | 数字 |
|---|---|
| **Yinta fork 实际版本** | `0.1.0` (package.json) — fork 自 v0.9.21 |
...
| **fork 跟 v0.9.21差距** | **估缺 75%** (446K / 1.75M, 估缺 1.3M LOC 闭源) |
...
| **v0.9.21 访问策略** | 选项 C 推存 (拿原版 NSIS, 解包, 1-2 周) |
| **8 项不修改承诺** | **8/8 0 触犯** |
| **6 哲学 anchor 穿透** | **6/6 0 漏** |

### 9.3 5 项立即建议 (R20 阶段 1-3)

1. **🔴 必做 1: 拿 v0.9.21原版 NSIS** (1-2 天) — 主人去 wei9966 官网或朋友拿, 走 7z + asar 解包, 产出 `new-unpacked-commercial/`, 估补 1.3M LOC 闭源
...
| `前代产品-branch-coverage-audit-2026-08-05.md` | §8 "Yinta fork = v0.9.21 + bypass" |
