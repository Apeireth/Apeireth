...
>
> **依据**: `APEIRETH-CONVENTIONS.md` 12 子规范 + `CHANGELOG.md` v2.0.0-alpha + `ROADMAP.md` R18+ 6 阶段 + `前代产品/docs/ARCHITECTURE.md` R19+ 集成 + `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` A 方案 + 9 份 `前代产品/reports/*.md` 分析。
>
...
**说明**:
- `apeireth-team-lead` 是 **R19+ 新 crate** (A 方案, 主人 2026-08-05 13:34 拍板), 翻译自 前代产品 supervisorPrompt.ts (970 LOC)
- `apeireth-session` 是 **R19+ 新 crate** (1500-2000 LOC), mid-task bug 3 处一起改
- 14 工具 = 8 调度 + 3 worktree + 3 感知 (来自 前代产品 AgentMCPServer.ts:893 LOC 翻译)
...
- `McpServer::from_registry()` 一行代码桥接 (前代产品 MCP 客户端零改造)
...
**关键约束**:
- `apeireth-git` (R19+ 新, 1000 LOC) 翻译自 前代产品 GitWorktreeService.ts:746 LOC
...
> **范围**: 9 份 `前代产品/reports/*.md` + 6 份 `docs/stage4/*.md` + 3 份 `docs/adr/0010-0012.md` = **18 份** 围绕 R19+ 集成的文档 (任务说 15 是 9+6, 加上 3 ADR 实际 18)。

```mermaid
flowchart TB
    subgraph TOP["核心蓝图 (3 份)"]
        A1[ARCHITECTURE.md<br/>前代产品/docs/<br/>前代产品→Apeireth 总览]
        A2[前代产品-integration-blueprint-r19-plus<br/>docs/stage4/<br/>A 方案 5 阶段]
        A3[tauri-roadmap-2026-08-05<br/>前代产品/reports/<br/>Tauri 阶段 13 项沉淀]
    end

    subgraph ADR["ADR (3 份 R19+ 新增)"]
        AD1[ADR-0010<br/>mcp-from-前代产品-agentmcpserver<br/>填 LOCKED 0 代码]
    ...
    subgraph REPORT["9 份 reports (前代产品/)"]
...
- **核心 3 蓝图**: `前代产品/docs/ARCHITECTURE.md` + `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` + `前代产品/reports/tauri-roadmap-2026-08-05.md`
- **3 ADR**: `docs/adr/0010-mcp-from-前代产品-agentmcpserver.md` + `0011-apeireth-team-lead-supervisor-prompt-translation.md` + `0012-team-lead-council-collaboration.md`
- **9 reports**: `前代产品/reports/{apeireth-{crate-api,platform-modules,protocol-4-adapter,council-7-advisor,graph-pipeline,mcp-14-tool,supervisor-tool-rules},tauri-roadmap,前代产品-architecture}-2026-08-05.md`
- **6 stage4 集成**: `docs/stage4/{apeireth-sdk-gap-analysis,apeireth-team-lead-implementation-guide,glossary-前代产品-additions,r-measure-verification-design,tauri-assets-from-前代产品,tauri-team-collab-sop}-2026-08-05.md`
...
- §4.4 mid-task bug 修法引用 前代产品 行号 (SessionManagerV2.ts:636-643), 这些是 TS 代码, Rust 端具体 crate 是 `apeireth-session` (R19+ 新), 行号需 rust-coder 接手时重测
