...
> - `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §4.4 (3 处组合根因 + 修法总表)
...
> - `前代产品/docs/ARCHITECTURE.md` §4 (mid-task bug 根因深挖)
>
...
| **mid-task bug 3 处修法必须改这个不存在的 crate** | `docs/ARCHITECTURE.md` §4 + `前代产品-integration-blueprint §4.4` 修法 1 明确写"`SessionManagerV2.sendMessage` → `apeireth-session/src/manager.rs::send_message`" | P0 阻塞,3 处一起改否则撕裂状态 |
...
| 原则 | 来源 | 落地 |
|------|------|------|
| **TS 翻译, 不 patch fork** | user memory #8 + 前代产品-integration-blueprint §1.3 | TS 源码当设计参考, Rust idiom 重写 |
...
> 前代产品 `SessionManagerV2.ts` = TS 端的"长会话调度器",跑了 19 个 Electron app 验证了 Supervisor-Member 团队协作,但 3 处 bug 撕裂状态。
>
...
edition = "2024"
description = "Apeireth 长会话调度子系统 (SessionManager V2 + 6 状态机 + mid-task bug 3 处根治) - R19+ 阶段 3 翻译自 前代产品 SessionManagerV2.ts"
license = "Apache-2.0 OR MIT"
repository = "https://github.com/apeireth/apeireth"
keywords = ["apeireth", "session", "lifecycle", "mid-task", "前代产品"]
...
/// SessionManager V2 — 长会话调度器 (翻译自 前代产品 SessionManagerV2.ts)
...
> **P0 必一起改**: 3 处是组合根因,任一不改 = 撕裂状态复发。来源: `docs/ARCHITECTURE.md` §4 + `前代产品-integration-blueprint §4.4`。

### 4.1 修法 1: send_message 状态机 (替代 throw 改 return)

**前代产品 根因** (`session/SessionManagerV2.ts:636-643`):
...
**前代产品 根因** (`agent/AgentManagerV2.ts:269-286`):
...
**前代产品 根因** (`agent/AgentManagerV2.ts:458-461` + line 88):
...
跟 ADR-0011 §不修改承诺 + APEIRETH-CONVENTIONS §10 + `前代产品-integration-blueprint §10` 一致:
...
- `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` (R19+ 集成蓝图总纲)
