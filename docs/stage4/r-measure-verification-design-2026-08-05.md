...
>
> **依据**: APEIRETH-CONVENTIONS §11 LOCKED 三值 (V1141=0.8682 / V1131=0.8532 / V1136=0.9063) + APEIRETH-COMPLETE-OMNIBUS-2026-07-30.md §3.5 V0.5 公式 + 前代产品-integration-blueprint-r19-plus-2026-08-05.md §6 守门 + r20-product-finalize-2026-08-05.md §6 守门.
>
...
| **V1136-R11** | **0.9063** | 真测引擎 7 子测度 (历史 R11 baseline, 当前实装扩到 9 子测度 per 前代产品 §1.1) | APEIRETH-CONVENTIONS §11 |
...
- **V1141/V1131 守门时**: 17 维通过 §2.1 投影公式从 24 维 LOCKED 算出 (per 前代产品 报告 §3.2 建议 B, **不动** apeireth-asi 24 维)
...
| **`apeireth-vector`** | 🟡 部分实装 (v2 新 crate) | **不**做 R-Measure, 存 embedding (任意维度, 768 维典型, per 前代产品 §1.1) |
...
├── fixtures/
│   └── r11on      (R11 baseline 数值, hardcode 等于 baseline.rs const, 估 30 LOC JSON)
└── examples/
...
判断:
  ① 真掉了 → 回滚或修代码, 重新跑 CI
  ② baseline 真的该升 → 写 ADR-00XX-baseline-bump, 主人拍板后才能改 fixtures/r11on
  ❌ 绝不绕过 (主 17:58)
```
...
**不重写**, 不假装"我也实现 V1136" (主 S-2 17:43 实验室). 直接调 apeireth-asi 公开 API. 24 维 LOCKED **不**动, 17 维投影在 verifier 端 (per 前代产品 报告 §3.2 建议 B).
...
### 6.1 必跑 verify 的 4 个触发点 (per r20-product-finalize §4 + 前代产品-integration-blueprint §9)
...
| **2** | 1 天 | 实施 `baseline.rs` (R11 3 值 hardcode const) + `fixtures/r11on` + `load_r11_baseline()` 守门 assert | 启动时守门 assert PASS, 改任何 JSON 字段启动失败 | rust-coder |
...
| **R-018** | **17→24 维 R11 baseline 投影公式 (v1077 权重) 未公开**: `v1077_asi_v04_full_measurement.py` 公式不公开, 17→24 维映射权重等主人确认 | 🔴 高 | 高 | ① 主人从 `v1077_asi_v04_full_measurement.py` 抽权重, 写到 `fixtures/r11-projection.json`; ② V1141/V1131/V1136 跑 verify 后跟 R11 baseline 对比 (≤ 0.001 tolerance); ③ apeireth-asi 24 维 LOCKED **不**动, 投影在 verifier 端 (per 前代产品 报告 §3.2 建议 B); ④ 阶段 0 必先完成权重抽取, 阶段 3 实装前必就位 | **P0** |

---

## §9 不修改承诺 (8 项, per APEIRETH-CONVENTIONS §10 + 前代产品 蓝图 §10)
...
| 3 | [`docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md`](前代产品-integration-blueprint-r19-plus-2026-08-05.md) | §6 (42 crate 总览) + §10 (不修改承诺) + §11 (风险清单) | R19+ 集成 4 触发点协同 |
...
| 7 | `crates/apeireth-r-measure-verify/fixtures/r11on` | 7 阶段阶段 2 实装 (R11 baseline 三值 hardcode) |
...
| **12** | **`crates/apeireth-asi/src/lib.rs` 加 `pub trait AsiEngine` + `snapshot_24dim()` async fn** | **🆕 阶段 0 必加 (per 前代产品 §3.3), 否则 verifier 调不动 24 维** |

### 11.3 引用 sub-agent 报告 (跟 Mavis 拍板有关)

| 报告 | 章节引用 | 关系 |
|------|---------|------|
| `前代产品-architecture-2026-08-05.md` | §6 (19 模块集成点) | 42 crate 集成背景 |
