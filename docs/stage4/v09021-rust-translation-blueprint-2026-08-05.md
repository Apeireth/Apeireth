...
> **必读输入 (9 份 + 1 当前实查)**:
> 1. `v09021-commercial-extract-2026-08-05.md` (250 行, 安装包分析实查)
...
> 6. `前代产品-branch-coverage-audit-2026-08-05.md` (572 行, 21 假盲点)
> 7. `yinta-fork-audit-2026-08-05.md` (504 行, fork 0.1.0)
> 8. `前代产品-integration-blueprint-r19-plus-2026-08-05.md` (R19+ 大蓝图)
> 9. `r20-product-finalize-2026-08-05.md` (R20 路线图)
> + 当前实查: `安装包out\main\` (171 .js / 452K LOC)
...
|---|------|------|
| §0 | 文档地图 (本节) | 任何人 |
| §1 | v0.9.21 1:1 翻译总体图 (总模块 / 总 LOC / 16 估缺 + 60+ SDK) | 任何人 |
...
| 源 | v0.9.21 `WorkflowGenerator-BQCQ_KQx.js` (~64KB) |
| 估 LOC | **1,500** |
| 估工时 | **4h** |
...
| 源 | v0.9.21 `ImagePromptLibrary` (~36KB) |
| 估 LOC | **600** |
| 估工时 | **1h** |
...
| 源 | v0.9.21 `RollbackService` (~22KB) |
| 估 LOC | **1,000** |
| 估工时 | **3h** |
...
| 源 | v0.9.21 `RepoScanAdapter` (~7KB) + `RepoAnalyzer` (~6KB) |
| 估 LOC | **800 + 700 = 1,500** |
| 估工时 | **2h + 2h = 4h** |
...
| 关键 API | `RepoScanner::scan` / `RepoAnalyzer::analyze` + `ScanResult` (per `ScanResult`) + `RepoProfile` (per `InstallErrorCodes-D-xTEXPe.js`) |
...
| 源 | v0.9.21 `keychain-token-storage` (~12KB) |
| 估 LOC | **400** |
| 估工时 | **1h** |
...
| 源 | v0.9.21 `getMachineId-` (7 文件, 每 4-5KB) |
| 估 LOC | **800** |
| 估工时 | **2h** |
...
| 3 | `apeireth-protocol::gemini` 增强 | `crates/apeireth-protocol/src/adapters/gemini.rs` (已有) | 800 (增量) | 4h | `GeminiAuthHelper` (per `GeminiAuthHelper` 13KB) + Gemini headless mode 完整 1:1 | `apeireth-protocol` (5 Provider 之一) |
| 4 | `apeireth-task` 增强 | `crates/apeireth-council/src/task.rs` (估补, 已有 task 估缺部分) | 1,100 (增量) | 4h | `TaskSessionCoordinator` + `taskTools` (per `taskTools` 313KB, 估 1500 LOC 估缺) | `apeireth-council` (任务状态机) + `apeireth-team-lead` (调度) |
| **总** | — | — | **2,200** | **10h** | — | — |
...
| 源 | Electron `out/main/index.js` (估 8K LOC) + `out/renderer/assets/index` (估 200K LOC) + Monaco / Sandpack / dnd-kit / Floating UI |
...
> **体检范围**: 跟 v0.9.21 1:1 对照, 找真实缺口 (不是文档缺陷).
...
| 8 调度工具 + 3 worktree + 3 感知 = 14 工具 | 估 14 工具 trait | v0.9.21 实查 14 工具 (per `前代产品-branch-coverage §4.i`) | ✅ 齐 (估, 需读全 src 验证) |
...
| TeamBus (per `commercial-vs-fork-diff §2.2`) | 8h | `apeireth-bus` L4 WebSocket + TeamBus 包装 (per `前代产品-integration §5.2`) |
| Orchestrator 增强 | 8h | 14 工具真接 apeireth-agent + 3 协同场景 (per `apeireth-team-lead` 阶段 1 实施) |
| supervisorPrompt 阶段 1 估缺补 (per `commercial-vs-fork-diff §3`) | 4h | paused + interrupted 9 状态补 (per `前代产品-branch-coverage §4.d`) + 134 行 markdown 1:1 翻译 |
...
| `apeireth-task` 增强 (§2.5.4) | 4h | TaskSessionCoordinator + taskTools 1:1 翻译 (per `taskTools` 313KB 估 1500 LOC) |
| 缓冲 | 4h | — |
| **阶段 4 总** | **60h** | — |
...
> **关键差异 (vs 原版预告)**:
> - 原版估缺 53h, RIVAL 版估缺 320h (5 周) — 因为 安装包分析后 16 估缺 + 5 增强 + 5 阶段 overhead 全摊开
...
```
[stage4-R20-1] 集成 5 P0 crate (v0.9.21 1:1 翻译)
...
| # | 模块 | 估时 | 怎么办 |
|---|------|-----:|--------|
| 1 | TeamRepository | 4h | **apeireth-memory** 加 1 repository (per `前代产品-branch-coverage §4.e` 12 repo 估缺 1) — R20 阶段 2 |
| 2 | TeamBus | 8h | **apeireth-bus L4 WebSocket + TeamBus 包装** (per `前代产品-integration §5.2`) — R20 阶段 2 |
...
| Anchor | 状态 | 实证 |
|--------|------|------|
| S-1 北极星导向 | 🟢 PASS | "全用 rust 1:1 翻译 v0.9.21 16 模块" (主人 19:37 强调) |
| S-2 实事求是 | 🟢 PASS | 安装包分析 1.4 GB / 171 .js / 452,173 LOC 实查 + 16 模块实查 + 5 P0 crate 体检实查 |
| O-2 走在前人肩上 | 🟢 PASS | v0.9.21 1:1 翻译 (不重设计, 不复用 TS) |
...
| v0.9.21 | 1.4 GB / 171 .js / 452,173 LOC (per 安装包分析) |
...
- v0.9.21 安装包分析工具 (7z 26.02 + @electron/asar)
...
2. 22 模块总图基于 v0.9.21 安装包分析实查 171 .js / 452,173 LOC + 9 份 stage4 文档引用, 不是凭空
