# GLOSSARY 前代产品 词条草稿（R19+ 待拍板）

```
[Document-Meta]
Document: docs/stage4/glossary-前代产品-additions-2026-08-05.md
...
>
> **依据**: `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §5.2 集成映射 + ADR-0010（apeireth-mcp 来自 前代产品 翻译）+ ADR-0011（apeireth-team-lead 新 crate）+ `docs/stage4/tauri-assets-from-前代产品-2026-08-05.md` 13 项资产清单。
>
...
**缩写/全称**:
- `前代产品`（全小写，作为来源项目代号；`` 目录命名风格）
- `前代产品`（PascalCase，文档/讨论时的正式写法）
...
**出处**: `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md` §1.1 + §3 现状

**关联**: `ARCHITECTURE.md` + `ADR-0010` + `ADR-0011` + `tauri-assets-from-前代产品-2026-08-05.md`

**注意**: 不要写 `前代产品` / `前代产品` / `spectral`（避免拼写漂移）。

---

## 🧭 2. team-lead / apeireth-team-lead

**定义**: R19+ 新 crate，**1:1 翻译** 前代产品 `supervisorPrompt.ts` (808 LOC) 的 Rust crate。负责构造"团队 leader 角色"的 system prompt + 触发 7 advisor voting。
...
**❌ 不要混淆**: council **不是** 前代产品 团队功能！council 是 R17 自创的"7 强制 advisor 平行审议"机制，跟 前代产品 无关。
...
**定义**: 前代产品 v0.9.21 已知的"**团队 leader 发 mid-task 消息给子 agent 失败**" bug。3 处根因（非 1 处）一起改才能根治。
...
**定义**: R19+ 集成的**副产品**——前代产品 Electron 特性沉淀成 Tauri 团队接手时的"项目资产"。

**13 项 T-001~T-013**:
- T-001 ~ T-013 涵盖 11 个 IPC handler + 1 个 UpdateManager + 1 个 OutputReaderManager 等
- 详细清单见 `docs/stage4/tauri-assets-from-前代产品-2026-08-05.md` §3

**文档位置**: `docs/stage4/tauri-assets-from-前代产品-2026-08-05.md`

**原则（3 项）**:
- **透明**: 13 项资产全部文档化，不留"隐藏资产"
- **文档化**: 每个资产有 T-XXX 编号 + 前代产品 来源文件 + LOC + 集成建议
- **不变成隐形资产**: Tauri 团队接手时能直接 grep 到 T-XXX 找到

**出处**: `tauri-assets-from-前代产品-2026-08-05.md`（R19+ 战略拍板文档）

**关联**: `ADR-0010`（apeireth-mcp 跟 前代产品 来源映射）+ 蓝图 §5.2 第 7 行
...
- `docs/stage4/前代产品-integration-blueprint-r19-plus-2026-08-05.md`（蓝图主文档）
- `docs/stage4/tauri-assets-from-前代产品-2026-08-05.md`（13 项资产清单）
- `ADR-0010`（apeireth-mcp 来自 前代产品 翻译）
