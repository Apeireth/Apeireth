# v0.9.21完整解剖 + 安装包分析实查 (主 19:37 拍板全用 rust 1:1 翻译)

```
[Document-Meta]
Document:    
Version:     Manual-Rev-A
R-Cycle:     v0.9.21完整解剖 (R20 阶段 1-3 实施基线)
Last-Modified: 2026-08-05
Status:      🎯 实查完成 (主 19:37 拍板解 NSIS + 全用 rust 1:1 翻译)
Author:      Mavis (per 安装包分析 + 7z + @electron/asar 工具)

> **性质**: v0.9.21 NSIS 完整解剖 + 60+ SDK 真实 deps + 8 估缺闭源模块实查 + 8 新发现模块. 主人 19:37 强调"全用 rust" 1:1 翻译, 不复用 TypeScript.
...
> **安装包分析路径**:
> 1. ` (1).exe` (438 MB, 2026-08-01)
> 2. → 7z x → `安装包\v0901\` ($PLUGINSDIR\app-64.7z 437 MB + $R0\Uninstall)
> 3. → 7z x → `安装包\v0901\app-64\` (前代产品.exe + resources\应用包 665 MB + 应用包.unpacked 780 MB)
> 4. → asar extract → `安装包` (out/main/ + out/renderer/ + package.json)
...
|---|------|-----:|---------|--------------|
| 1 | TeamRepository | 4h | `TeamRepo-*` 估 | apeireth-memory 加 12 repository (per D §5.e) |
| 2 | TeamBus | 8h | `TeamBus-*` 估 | apeireth-bus L4 WebSocket + TeamBus 包装 (per blueprint §5.2) |
| 3 | TaskKanban | 16h | `TaskKanban-*` 估 + dnd-kit | apeireth-council::task + Tauri 阶段 (per D-12) |
| 4 | Orchestrator | 13h | `out/main/agent/AgentMCPServer.js` 部分 | apeireth-team-lead 850 LOC (per A 方案 13:34 拍板) |
| 5 | AutonomousPlanner | 24h | `AutonomousPlanner-*` 估 | apeireth-asi 24 维 + autoDream 4 阶段 (per R20 中期 1 月) |
| 6 | TelegramBotManager | 8h | `TelegramBot-*` 估 | Discord 冷启动 (per D-12) + R21+ 商业化 |
| 7 | AIRouter | 6h | `AIRouter-*` 估 | apeireth-protocol 5 base URL + AIRouter (per 5-provider-tool-mapping §2.7) |
| 8 | SuggestionEngine | 16h | `SuggestionEngine-*` 估 | Tauri 阶段 T-006 + R21+ UX |
| **估缺总估时** | — | **95h** | — | — |
...
| 12 | **WorkflowGenerator** | `WorkflowGenerator-*` 估 | 估 1500 | 4h | apeireth-workflow (新) |
| 13 | **PluginManager** | `PluginManager-*` 估 | 估 800 | 2h | apeireth-plugin (新) |
| 14 | **ImagePromptLibrary** | `ImagePromptLibrary-*` 估 | 估 600 | 1h | apeireth-image-prompt (新) |
| 15 | **RollbackService** | `RollbackService-*` 估 | 估 1000 | 3h | apeireth-rollback (新) |
| 16 | **RepoScanAdapter + RepoAnalyzer** | `RepoScanAdapter-*` + `RepoAnalyzer-*` 估 | 估 1500 | 4h | apeireth-repo-scan + apeireth-repo-analyzer (2 新) |
| 17 | **tree-sitter-bash + tree-sitter-Fukzi** | `tree-sitter-bash-*` + `tree-sitter-Fukzi*` 估 | 估 2000 | 6h | apeireth-tree-sitter (已有, 增强) |
| 18 | **keychain-token-store** | `keychain-token-store-*` 估 | 估 400 | 1h | apeireth-keyring (新) |
| 19 | **getMachineId-{win,darwin,bsd,linux}** | `getMachineId-*` 估 | 估 800 | 2h | apeireth-machine-id (新) |
| **新发现总估时** | — | — | — | **37h** | — |
...
| WorkflowGenerator-* | 估 1500 | apeireth-workflow |
| PluginManager-* | 估 800 | apeireth-plugin |
| ImagePromptLibrary-* | 估 600 | apeireth-image-prompt |
| RollbackService-* | 估 1000 | apeireth-rollback |
| RepoScanAdapter + RepoAnalyzer-* | 估 1500 | apeireth-repo-scan + apeireth-repo-analyzer |
| tree-sitter-bash + Fukzi | 估 2000 | apeireth-tree-sitter (增强) |
| keychain-token-store-* | 估 400 | apeireth-keyring |
| getMachineId-{4 平台} | 估 800 | apeireth-machine-id |
| taskTools-* + stdio-* + multipart-parser-* | 估 1500 | apeireth-task (新) |
| GeminiAuthHelper-* | 估 600 | apeireth-protocol::gemini (增强) |
...
| Anchor | 状态 | 实证 |
|--------|------|------|
| S-1 北极星导向 | 🟢 PASS | "全用 rust 1:1 翻译 v0.9.21" (主人 19:37 强调) |
...
|---|---|
| 路径 | `` |
| 安装包分析 | 1.4 GB (应用包 665 MB + unpacked 780 MB) |
| 总 .js | 171 文件 |
| 总 LOC | **452,173** (per E 估 446,652 ± 1.2%) |
...
**S-2 实事求是登记**:
1. 本报告纯实查, 不写代码, 不 git commit (产出物在 前代产品 工作树)
2. v0.9.21 实查数据基于 安装包分析 1.4 GB / 171 .js / 452,173 LOC, 跟 E 估 446K ± 1.2%
3. 16 模块实查基于 `out/main/` + `out/main/agent/` + `out/main/mcp/` + `` 目录实查, 不是凭空
