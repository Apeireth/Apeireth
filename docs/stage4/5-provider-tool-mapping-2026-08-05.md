# 前代产品 5 Provider × toolMapping 详细分析 + apeireth-protocol Rust 设计
...
> **数据源**:
> - `` (259 LOC, 全 5 Provider 集中映射)
> - 5 个 Adapter 源 (per `前代产品-architecture-2026-08-05.md` §2 模块 #2 + `apeireth-protocol-4-adapter-analysis-2026-08-05.md` §1)
> - `` (providerId ↔ MCP/Skill 能力)
...
> **数据源**: `` (`ActivityEventType` type alias)
...
### §4.1 minimax m3 / MiniMaxi 在 前代产品 5 Provider 里吗? (per 主人 17:33 砍 D-01)

**答案**: ❌ **不在**. minimax m3 是 **Apeireth 自有 LLM** 走 `apeireth-api`, 不在 前代产品 5 Provider 之列.

**证据 1**: 5 个 adapter 源 grep 结果 (per `bash` 工具实际跑)
```powershell
PS> Select-String -Path "...\前代产品-source\src\main\adapter" -Pattern "minimax|MiniMax|M3|DeepSeek|Qwen|Alibaba" 
(0 matches)
```
...
|------------|--------|------|----------|------|
| **前代产品 5 Provider** | TS Electron 桌面层 (per `adapter/toolMapping.ts`) | 5 个 LLM CLI 工具的 SDK 适配 | `ProviderCapabilityRegistry.ts` | SDK 协议 (VCP 借鉴) |
...
- **apeireth-protocol 4 ZST adapter** = OpenAI Chat / OpenAI Responses / Anthropic Messages / Gemini (跟 前代产品 5 Provider 的 Claude/Codex/Gemini 部分重叠)
...
- m3 进 前代产品 = **当前不在 5 Provider**, 主人 17:33 砍 D-01 决策后, **m3 走 Apeireth → TUI, 不走 前代产品**

**建议** (本报告新增, per `apeireth-protocol-4-adapter-analysis §1.2` + `r20-stage-1-prep §4.1`):
- ✅ m3 **保持不进 前代产品 5 Provider** (per 主人 17:33 砍 D-01 决策)
...
**注册入口**: `AdapterRegistry` (per `前代产品-architecture §3.1` + `r20-stage-1-prep §1.3` 协同点 #2 "5 Provider base URL 配齐"):
```typescript
// bootstrap/index.ts 启动流程 (per 前代产品-architecture §4.1)
...
- 🟡 **m3 extension 预留**: 即使 m3 不进 前代产品 5 Provider, `toolMapping.ts` 留 1 个占位 `'m3'` case 兜底, 未来真加时再写 (per O-2 不漂移, 不假装已实装)
...
**全局重试配置**: **缺失**. 前代产品 当前没有 Provider-level maxRetries 配置, retry 是**写死在 ClaudeSdkAdapter 内部** (5s hardcode).
...
//!
//! 数据源: 前代产品 `toolMapping.ts:11-131` (5 HashMap, 63 entries)
//! 翻译哲学: 保 raw + truncate, 不让 LLM 二次解释 (per O-5 不假装)

/// 5 Provider enum 变体 (1:1 对应 前代产品 providerId)
...
**新事件类型** (per `ProviderEvent` 在 `前代产品-architecture §3.1` 已有 `ProviderEventType` 11 变体):
...
> **关联文档**:
> - `` (70KB, 6 anchor × 5 验证项 + 5 fixture + serve SOP)
> - `` (73KB, 6 端点 + WebSocket + Docker + 1.0 release)
...
- 5 base URL 配齐 (per `r20-stage-1-prep §4.1` Step 1.2): OPENAI / OPENAI_RESPONSES / ANTHROPIC / GEMINI / MINIMAXI — **跟 前代产品 5 Provider 不冲突** (per §4.1 对账表)
...
| 3 | `/v1/tools/git_ops/invoke` | POST | `apeireth-tools::GitTool::call` | 🟡 **toolMapping 无 git 工具**, 推测 前代产品 走 `command_execute` 兜底 (`git` 命令) |
...
- 直接复用 前代产品 `toolMapping.ts` (259 LOC, 63 entries)
...
- pyo3 桥 (per `apeireth-pybridge` 已实装 859 LOC, per `前代产品-architecture §3.2`)
...
**严守方式**:
- 本报告**纯文档**, 输出路径 ``, 在 Mavis 自己的 reports 子目录, 不污染 Apeireth 主项目
...
| 5 | **O-3** 干到底 (边界清晰) | 🟢 PASS | 本报告只动 `reports/前代产品-r19plus-v2/`, 不碰 `crates/apeireth-*/src/`; 跟 Hermes 5 协同点 0 冲突 (per `r20-stage-1-prep §1.3` + `r20-stage-2-3-prep §1.5`) |
| 6 | **O-4** 任何人都能接手 (文档化 + 编译期 hardcode) | 🟢 PASS | 本报告 §2 5 HashMap 全列 + §3 ActivityEventType 24 变体全列 + §5 Rust 设计 5 子节详细, 后续接手者不用再读 前代产品 源; 5 HashMap 63 entries 编译期 hardcode (per §5.4) |
...
| 9 | 文档路径 (per CONVENTIONS §1) | ✅ 本报告放 前代产品-r19plus-v2/ 子目录, 跟 R20 准备文档 (r19-integration-v2/) 一致 |
...
- sub-agent 1 architect 写的 `前代产品-architecture-2026-08-05.md` §2 + §5.2 (5 Provider 完整映射表)
...
- 前代产品 weibin 写的 `toolMapping.ts:259 LOC` 全部 63 entries
