# 前代产品 Branch Coverage 审计 + 假盲点扫除报告

**生成日期**: 2026-08-05
**审计范围**: 前代产品 0.4.6 git upstream (v0.9.21 NSIS 拆解产物 + `前代产品-source/`)
**审计人**: 前代产品 branch coverage 审计 + 假盲点扫除 sub-agent
**任务编号**: R20 阶段 1
...
| 项 | 值 |
|---|---|
| **目标** | 扫除**真盲点** (前代产品 release/beta branch) + **假盲点** (形式有但实质可能漏) |
...
| **配套上下文** | ① `前代产品-architecture-2026-08-05.md` (920 行) ② `tauri-roadmap-2026-08-05.md` (32.4 KB, 13 项 T-001~T-013) ③ `apeireth-crate-api-2026-08-05.md` (44.3 KB, 10 crate) ④ `apeireth-supervisor-tool-rules-2026-08-05.md` (44.9 KB) |

### 0.2 实跑 git 命令 (本报告所有 "git 输出" 都是真跑)

| 命令 | 实际结果 |
|---|---|
| `git -C 前代产品-source branch -a` | 1 local + 2 remote, **0 release / beta / dev / feature branch** |
| `git -C 前代产品-source log --oneline --all -50` | **2 个 commit** (Initial + 1 个 docs 改动) |
| `git -C 前代产品-source tag` | **0 tag** |
| `git -C 前代产品-source rev-list --all --count` | **2** |
| `git -C 前代产品-source rev-parse --is-shallow-repository` | **true** (浅克隆) |
| `git -C 前代产品-source remote show origin` | upstream = `https://github.com/wei9966/前代产品.git` |
| `git -C 前代产品-source fetch --unshallow` 后再跑 | 仍然 **2 commit** (说明 upstream 真的只有 2 个 commit) |

---

# Part 1: 真盲点 (前代产品 branch 审计)
...
```bash
$ git -C 前代产品-source log --oneline --all -50
ba0cf45 docs: remove test line and add official website link
6cb3324 Initial commit: 前代产品 Community Edition
```

```bash
$ git -C 前代产品-source tag
(空)
```

```bash
$ git -C 前代产品-source remote show origin
* remote origin
  Fetch URL: https://github.com/wei9966/前代产品.git
  Push  URL: https://github.com/wei9966/前代产品.git
  HEAD branch: main
  Remote branch:
    main tracked
...
| upstream repo | 未知 | `wei9966/前代产品` (单分支单作者 repo) |
...
|---|---|---|---|
| `6cb3324` | weibin <996649855@qq.com> | 2026-04-04 16:40 +0800 | "Initial commit: 前代产品 Community Edition" |
...
| 维度 | NSIS v0.9.21 (主人 reverse engineer) | git HEAD v0.4.6 (前代产品-source) |
|---|---|---|
...
| **来源** | `安装包 (1).exe` 428 MB (wei9966 官方) | github.com/wei9966/前代产品 OSS (社区版) |

**架构报告口径错位**:
- `前代产品-architecture-2026-08-05.md` 标题写 "**前代产品 v0.9.21**"
- 但所有源码路径引用 `前代产品-source/src/main/...` (v0.4.6)
...
| 应用包 大小 | 661 MB | 697 MB | 不适用 (无 asar) |
...
主人在 `前代产品-architecture-2026-08-05.md` §1 写"主分支 source as 80% baseline"。基于 `前代产品-source/` (v0.4.6) 的源码分析, **剩下 20% 是**:
...
**关键观察**: 这些缺失项, **在 `前代产品-source/README.md` 第 6 章"架构特性"里有大段描述** (TeamManager, TaskKanban, TeamMessageFlow, Workflow, AutonomousPlanner, Telegram Bot), 但 git 仓库源码里**完全没有**。说明:

- wei9966 把 前代产品 0.4.6 当作 **"社区版" OSS 首次提交**, 故意省略了 Teams/Workflow/Telegram/Planner 等闭源模块
- v0.9.21有 ~1.75M LOC, 是社区版 25K LOC 的 **70 倍**
...
1. **🔴 v0.4.6 社区版 ≠ v0.9.21** — 主人以为"看了 80% main", 实际只看了 **社区版 100% + 商业版 0%**。商业版 Teams/Workflow/Telegram 等闭源模块完全没源码, 主人也**没意识到这个 gap**。
2. **🔴 架构报告标题错位** — `前代产品-architecture-2026-08-05.md` 标题写 "v0.9.21", 但所有源码引用 v0.4.6, 主人 / 拍板者没发现这个错位。
3. **🟡 主人 reverse engineer 的产物 (`安装包 (1).exe`) 来自 Yinta fork** — `new-unpacked/package.json` 的 `fork.fromVersion: 0.9.21` 标识说明: 主人 2026-08-03 装上的是 chuling@local 的 Yinta fork, 不是 wei9966 原版 前代产品 0.9.21 binary。
4. **🟡 upstream repo 不可信** — `wei9966/前代产品` git 仓库只有 2 commit, 跟 NSIS 实际内容 (v0.9.21 / 1.75M LOC) 严重不一致。后续如果有人想从 git pull 来 build, 拿到的会是 v0.4.6 社区版, 不是 v0.9.21。
...
| **🟡 P1** | 架构报告标题错位 | 主人应该**手动修正 `前代产品-architecture-2026-08-05.md` 第 4 行**, 把 "v0.9.21" 改成 "v0.4.6 社区版 + v0.9.21 NSIS" |
| **🟡 P1** | Yinta fork 误标为 前代产品 | 主人应该**澄清 NSIS installer 真正来源** (是 wei9966 原版还是 chuling Yinta fork) |
| **🟢 P2** | upstream repo 不可信 | 后续 build 不要从 git pull, 直接用 安装包分析产物 |
...
**实查**: `前代产品-source/src/main/adapter/` 共 9 文件, 5 个 Provider 类 (`ClaudeSdkAdapter` / `CodexAppServerAdapter` / `GeminiHeadlessAdapter` / `IFlowAcpAdapter` / `OpenCodeSdkAdapter`) + `ProviderCapabilityRegistry` (静态能力表) + `AdapterRegistry` (路由) + `types.ts` (基类) + `toolMapping.ts` (事件映射)。
...
**实查** (`前代产品-source/src/main/parser/`):
...
**实查** (`前代产品-source/src/main/storage/repositories/`):
...
**实查** (`前代产品-source/src/main/ipc/`, grep `ipcMain\.handle`):
...
```
22 工具实际清单 (排除 server name '前代产品-agent'):
1. spawn_agent
2. send_to_agent
3. get_agent_output
4. wait_agent_idle
...
- 4 文件 (前代产品_edit_file / 前代产品_write_file / 前代产品_create_file / 前代产品_delete_file) — 报告**完全漏掉文件类**
...
**结论**: 🟡 **形式有 retry 提及, 实质只 1 Provider 1 处 retry** — sub-agent 1 architect 提"5 Provider 各自的 retry 配置"是空想, 实际 4 Provider 没 retry。`apeireth-protocol` 集成时需要**自己加** backoff, 不能从 前代产品 翻译。
...
| 2 | 不改 crates/apeireth-*/src/ | ❌ **0 触犯** | 本审计**只读** `前代产品-source/src/main/**.ts` 源文件 (前代产品 项目, 不是 Apeireth), **0 写操作** |
...
| 7 | 产出物只写到 `前代产品-r19plus-v2/` | ✅ **遵守** | 本审计**只写** 1 个文件 `前代产品-branch-coverage-audit-2026-08-05.md` 到 ``, 不污染其他目录 |
...
## §8 Yinta fork 跟 v0.9.21对比 (per sub-agent E yinta-fork-audit-2026-08-05.md, 2026-08-05 19:30)

### §8.1 fork = v0.9.21 + paid tier bypass (per E §1)

| 维度 | Yinta fork (实查) |
|------|-------------------|
| fork `package.json` | `"version": "0.1.0"`, `"fork.fromVersion": "0.9.21"`, `"author": "chuling@local"` |
| fork 时间 | 2026-08-03 |
| 实际改动 | `out/renderer/assets/index` (paid tier bypass) |
| 改动内容 | `getEffectivePlan()` 永远返回 `enterprise` |
| 818 行 supervisorPrompt | 估保留 (per E §7.1 估) |

**结论**: Yinta fork 不是从 v0.4.6 fork, 是从 **v0.9.21 fork** + 主人自己改 paid tier 旁路。

### §8.2 446K LOC vs 1.75M LOC 商业版 = 估缺 75% (1.3M LOC 闭源)

| 维度 | v0.4.6 社区版 | v0.9.21 | Yinta fork (估) | 跟商业版差 |
|------|---------------|----------------|-----------------|------------|
...
| **C. 拿原版 安装包分析** | 1-2 周 | +1.3M LOC 估 | 0 (per E §5) | ✅ |

**推荐 C 路径**:
- 主人 v0.9.21 NSIS 在 `` (per E §5 估)
- 安装包分析: `7z x 安装包 -o前代产品-Commercial-0.9.21/` (per Yinta fork 解包经验)
...
**修订**: 不以 v0.4.6 社区版 (缺 75% 商业版) + 不以 v0.9.21 (闭源访问受限) = **以 Yinta fork + 原版 NSIS 联合** (1-2 周后)。
...
**报告人**: 前代产品 branch coverage 审计 + 假盲点扫除 sub-agent
**交付时间**: 2026-08-05
