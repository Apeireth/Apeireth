...
**源文件**: `` (808 LOC, 7 个 build*() 函数 + 7 个 inject*() 入口 + 2 个 helper, 40 个 markdown 标题)
...
- ✅ **只读** `前代产品-source/src/main/agent/supervisorPrompt.ts` 808 行全部源码(已读)
...
| 已有报告 | 覆盖范围 | 本报告增量 |
|---------|---------|----------|
| `前代产品-architecture-2026-08-05.md` §5.2 | supervisorPrompt → apeireth-team-lead 1 行映射 | 本报告提供**逐函数 / 逐章节** 拆解 |
...
|---|---------|-------|------------|------------------|------------|-------|
| 1 | **感知层 (Awareness)** | 27-51 | 18 行 | `injectAwarenessPrompt` (241-250) + `cleanupSupervisorPrompt` (288-297) | `.claude/rules/前代产品-session.md` | **P0** |
| 2 | **调度层 (Supervisor)** | 53-218 | 158 行 | `injectSupervisorPrompt` (255-283) | `.claude/rules/前代产品-session.md` | **P0** |
...
| 5 | **文件操作规范 (FileOps)** | 422-487 | 42 行 | `injectFileOpsRule` (480-487) | `.claude/rules/前代产品-fileops.md` | **P0** |
| 6 | **Worktree 隔离规范** | 489-675 | 56 行 | `injectWorktreeRule` (654-662) + `cleanupWorktreeRule` (667-675) | `.claude/rules/前代产品-worktree.md` | **P1** |
| 7 | **Worktree 已激活** | 573-608 | 15 行 | `injectWorktreeAlreadyActiveRule` (602-608) | `.claude/rules/前代产品-worktree.md` | **P1** |
...
- **"前代产品" 字样** → 翻译为 "apeireth"(主人决策: 重命名,见 sub-agent 1 architect §5.1 命名空间冲突表)
- **"Claude Code" 字样保留**(per sub-agent 1 architect §5.2 minimax m3 识别)
- 注入路径:`.claude/rules/前代产品-session.md` → apeireth-team-lead 翻译为 `.apeireth/rules/team-lead-session.md`(或保持兼容 .claude/rules/ 因为 Claude Code 实际加载)
...
5. **⚠️ 工具预加载(必做)**:前代产品 调度工具可能处于 **deferred** 状态,使用前**第一步**必须 `ToolSearch(query: "+前代产品-agent spawn")` 预加载
...
10. **spawn_agent vs 内置 Task 工具**:需要选择不同 AI Provider / worktree 隔离 / 多轮交互 / 前代产品 进度追踪 → 优先 spawn_agent;简单只读搜索 → 直接用 Grep/Read/Glob 或内置 Task
...
- **"前代产品" 平台名** → 翻译为 "apeireth" (但 m3 兼容性测试可能要保留,见 §4.2)
- **`ToolSearch(query: "+前代产品-agent spawn")` 步骤** — Rust 端 1:1 保留(这是 Claude Code 工具发现协议)
- **`前代产品_spawn_agent` / `前代产品_wait_agent_idle` 等 MCP 工具名前缀** → 翻译为 `mcp__apeireth-agent__spawn_agent`(per architect §5.2 mcp::config_gen 翻译表)
...
**章节功能**:**最高优先级**规范,强制 AI 使用 前代产品 MCP 文件操作工具(而非 Claude Code 内置工具)来修改文件。这是主人 m3 测出来的关键修复(见 §4.1)。

**verbatim 标题清单**:
- `# 前代产品 文件操作规范(最高优先级)` (line 432 — H1 标题,因为是独立文件)
- `## 强制规则` (line 436)
- `## 工具参数说明` (line 449)
- `### 前代产品_edit_file(替代 apply_patch / Edit)` (line 451)
- `### 前代产品_write_file(替代 Write / 覆写式 apply_patch)` (line 456)
- `### 前代产品_create_file(替代新建文件的 apply_patch)` (line 460)
- `### 前代产品_delete_file` (line 464)
- `## 重要说明` (line 467)
...
| 操作 | 必须使用 | 禁止使用 |
|------|---------|---------|
| 编辑文件 | `前代产品_edit_file` | Edit, apply_patch, sed, awk, patch |
| 写入/覆写 | `前代产品_write_file` | Write, cat >, echo >, apply_patch |
| 创建新文件 | `前代产品_create_file` | Write, touch, apply_patch |
| 删除文件 | `前代产品_delete_file` | rm, del, unlink |
...
- **核心目的** (line 471): 前代产品 平台精确追踪每次文件改动并展示 diff
...
- **MCP 工具名前缀** `mcp__前代产品-agent__前代产品_edit_file` → Rust 端 `mcp__apeireth-agent__apeireth_edit_file`(per architect §5.2 mcp 翻译)
...
- **注入路径**:`.claude/rules/前代产品-fileops.md` (独立文件,与 前代产品-session.md 分离)
...
- **`enter_worktree` 工具名** → Rust 端保留(对应 apeireth-mcp 的 `前代产品_enter_worktree` 工具)
...
- **2 个 ❌ 禁止** (line 589-590):不要调 `enter_worktree` / 不要手动合并(前代产品 调度器统一合并)
- **branchName 动态注入** (line 580, 596):`前代产品 平台已为此会话自动创建了隔离的 Git Worktree 分支(\`${branchName}\`)`
...
- **"由 前代产品 调度器在任务完成后统一合并"** → 翻译为 "由 apeireth 调度器在任务完成后统一合并"
...
**功能切片**:
- **5 个 helper**:
  - `blockMarkers(blockId)` (line 680-685): 生成 前代产品 管理块标记 `<!-- CLAUDEOPS:WORKTREE:START/END -->`
...
|------|-------|-----------|---------|---------|----------|---------------------|
| **buildAwarenessPrompt** ⭐ | 27-51 | 18 | `.claude/rules/前代产品-session.md` | 通用(所有 Claude Code 会话) | 3 个感知 | `AWARENESS_PROMPT: &str` |
| **buildSupervisorPrompt** ⭐ | 53-218 | 158 | `.claude/rules/前代产品-session.md` | Supervisor 模式 | 14 个调度 + 3 worktree | `SUPERVISOR_PROMPT: &str` |
| **buildFileOpsPrompt** ⭐ | 422-487 | 42 | `.claude/rules/前代产品-fileops.md` | 全局强制 | 4 个文件操作 | `FILE_OPS_PROMPT: &str` |
| **buildWorktreePrompt** | 489-675 | 56 | `.claude/rules/前代产品-worktree.md` | autoWorktree on | (enter_worktree 是 Claude 内置) | `WORKTREE_PROMPT_TEMPLATE: &str` |
| **buildWorktreeAlreadyActivePrompt** | 573-608 | 15 | `.claude/rules/前代产品-worktree.md` | autoWorktree 已触发 | — | `WORKTREE_ALREADY_ACTIVE_TEMPLATE: &str` |
...
**关键设计点**:
- 注入到 `.claude/rules/前代产品-session.md` (Claude Code 官方规则发现路径)
...
- **"前代产品" 字样** → "apeireth"(per architect §5.1 命名空间冲突),但 m3 兼容测试可能要保留 — 见 §4.2
...
- 4 个强制工具表(`前代产品_edit_file` / `前代产品_write_file` / `前代产品_create_file` / `前代产品_delete_file`)
- `apply_patch` 特别强调(主人 m3 测出的关键修复)
- "读取不受约束" / "Bash 写文件不受约束" 例外明确

**Rust 端翻译关键**:
- 42 行适合 `include_str!("file_ops.md")` + 常量
- **MCP 工具名前缀翻译**: `mcp__前代产品-agent__前代产品_edit_file` → `mcp__apeireth-agent__apeireth_edit_file` (per architect §5.2 mcp 翻译)
...
| # | 章节 | 源码行 | m3 测出问题 | 修复方式 |
|---|------|-------|-----------|---------|
| 1 | **文件操作规范 (buildFileOpsPrompt)** | 422-487 | AI 用 `apply_patch` 改文件,平台 diff 追踪失败 | **强制 前代产品_* MCP 工具**, `apply_patch` 列入禁止 |
...
**"前代产品" 字样 vs "Claude Code" 字样 — 翻译策略不同**:

- **"前代产品" 平台名** → 翻译为 "apeireth" (per architect §5.1 命名空间冲突,这是**主人在 2026-08-05 拍板的命名重命名**)
- **"Claude Code" 工具名** → 1:1 保留 (per architect §5.2 minimax m3 兼容性,这是**主人在 2026-08-05 拍板的兼容保留**)

**Rust 端 1:1 翻译时区分**:
- `pub const AWARENESS_PROMPT: &str` 中"前代产品" → "apeireth", "Claude Code" → "Claude Code"
...
| 位置 | m3 适配 | 源码行 |
|------|---------|-------|
| **工具预加载** `ToolSearch(query: "+前代产品-agent spawn")` | 解决 m3 "工具名混乱" 问题 | line 79-88 |
...
### 4.4 主人在 前代产品 上迭代的痕迹(从 git log 推断,见 §0 边界声明)
...
**作用**: 这是 apeireth-team-lead 比 前代产品 强的地方(根据主人实测经验加防御,不来自 818 行原文)。

#### 5.2.2 m3 工具发现协议适配

818 行 §2.2.5 `ToolSearch(query: "+前代产品-agent spawn")` 是 Claude Code 工具发现协议,minimax m3 也认。
...
| 实际改动 | `out/renderer/assets/index` (per E §1) |
| 改动内容 | `getEffectivePlan()` 永远返回 `enterprise` (per E §1 + 商业版付费墙对照) |
| 818 行 supervisorPrompt | fork 0 改动 (per E §2.7 估, prompt 内容跟 v0.9.21同) |
| Claude 字样保留 | 估保留 9 处 (per §7.1 估) |

**结论**: Yinta fork = v0.9.21 + paid tier bypass。supervisorPrompt 818 行内容**0 改动**, 只是 paid tier 解锁。

### §7.2 1:1 翻译 "前代产品" → "apeireth" (主人拍板)

per sub-agent 1 architect §5.1 命名空间冲突 (per C §6.2):
- "前代产品" 平台名 → "apeireth" (Rust 端, apeireth-team-lead 命名)
- "前代产品-agent" MCP server 命名 → "apeireth-mcp::team" (per apeireth-mcp-14-tool-analysis §1.2)
- "前代产品" 团队名 → "apeireth-council" (per §5.1 已有冲突)
...
- O-5 不假装 = 0 paid tier 翻译 (跟 v0.9.21不同)
...
- O-4 任何人都能接手 = §7.1 fork 实查 + §7.3 0 paid tier 翻译清晰可读
    ...
    #[test]
    fn fn_fileops_preserves_前代产品_prefix() {
        let prompt = build_file_ops_prompt();
        // MCP 工具名前缀保留
        assert!(prompt.contains("前代产品_edit_file"), 
                "K-1: FILE_OPS_PROMPT must preserve '前代产品_edit_file' tool name");
...
4. SUPERVISOR_PROMPT 包含 `ToolSearch(query: "+前代产品-agent spawn")` 1:1 保留
5. FILE_OPS_PROMPT 包含 4 个 前代产品_* 工具名(edit/write/create/delete)
...
| # | 待澄清点 | 建议 |
|---|---------|------|
| 1 | "前代产品" 平台名是否 100% 翻译为 "apeireth"? | 建议**保留** "前代产品" 作为历史标注 + 翻译为 "apeireth" — 这样 m3 兼容性测试有 fallback(主人 m3 见过 前代产品) |
| 2 | `${availableProviders.join(', ')}` 注入顺序是否固定? | 建议**固定**为 `claude-code, gemini-cli, codex, opencode` (per §2.2 fallback 顺序) |
| 3 | `.claude/rules/前代产品-session.md` 注入路径是否改成 `.apeireth/rules/`? | 建议**保留** `.claude/rules/` 路径(Claude Code 实际加载,改路径会破坏加载) |
...
- **Fixture 2 `test_supervisor_prompt_injection`** 验证 .claude/rules/前代产品-session.md 注入流程
- **Fixture 3 `test_file_ops_mcp_tools`** 验证 前代产品_* MCP 工具强制
...
| 交付物 | 路径 | 大小 |
|--------|------|------|
| **supervisorPrompt 818 行章节摘要** | `` | (本文件,~700 行) |
...
| 引用 | 路径 | 用途 |
|------|------|------|
| sub-agent 1 architect 报告 | `` | §0 边界声明 / §1 顶层摘要 / §2 19 模块 / §3 架构图 / §5.2 supervisorPrompt 映射(本报告引用并增量) |
| sub-agent 1 architect 报告 §5.2 | (同上) §5.2 | "主人 3 决策: 1:1 翻译" / "Claude Code / Claude 字样保留" / 新命名建议 `apeireth-team-lead` |
| sub-agent 1 architect 报告 §5.1 | (同上) §5.1 | 命名空间冲突表(前代产品 "supervisor" ≠ Apeireth `apeireth-supervisor`) |
| tauri-roadmap 报告 | `` | 13 项 Tauri 阶段沉淀(supervisorPrompt **不在**清单,TUI 现在做) |
| apeireth-supervisor-tool-rules | `` (45940 bytes) | supervisor 工具规则分析(本报告聚焦 supervisorPrompt.ts 内部) |
| R20 阶段 1 准备 | `` (71760 bytes) | Fixture 1 `test_team_lead_workflow` |
| R20 阶段 2/3 准备 | `` (73010 bytes) | 6 端点(calendar / message / team_lead / agent / session / provider) |
| supervisorPrompt.ts 源文件 | `` (808 LOC) | 818 行 markdown 章节摘要源(已读全部 808 行) |
