...
| 50 | target 构建缓存治理长效机制 | 主人 2026-08-17 指示 (内存/磁盘不无限膨胀) | 基线实测: target 45.9 万文件/269.6 GB, debug/incremental 105.28 GB 可回收; scripts/target-hygiene.ps1 (robocopy /L 1.1s 测量, 默认报告 -Apply 才删, 不碰 deps/build/.fingerprint) + maintenance-guide §五 治理规范 (时机规则: 禁活跃编译期全清) | ✅ 任务 3c38e55f: 脚本实跑留基线证据 (报告模式); 真清 (105 GB) 留静默窗口由 Leader 批准后 -Apply |
