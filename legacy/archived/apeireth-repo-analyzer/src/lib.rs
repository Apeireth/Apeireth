...
//!
//! Repo Analyzer (1:1 翻译 v0.9.21 `RepoAnalyzer` ~6KB).
//! 商业版 RepoAnalyzer 是 **代码质量 + 技术债 + 复杂度 + 依赖 + 安全审计 + 报告生成** 一体化工具,
//! 通过 `RepoScanAdapter` 共用扫描结果 (per `v09021-commercial-extract §3.3`).
//! 本 crate 1:1 翻译 + 扩展 7 个核心能力, R20 阶段 1 续 P1 估缺.
//!
//! ## v0.9.21 RepoAnalyzer 实查 (obfuscated webpack bundle, 6KB, 单行)
//!
//! | Token | 命中 | 推断 | 出处 |
//! |-------|-----:|------|------|
...
///
/// v0.9.21估缺 schema 校验, R20 阶段 1 续补. 工具调用必须满足:
