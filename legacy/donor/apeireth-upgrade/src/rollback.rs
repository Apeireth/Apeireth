//! # apeireth-rollback
//!
//! **Rollback Service** (1:1 翻译 v0.9.21
//! `RollbackService` ~22KB).
//!
...
//!
//! **事故根因**:
//! 前代产品 v0.9.21有个 bug: `agent sandbox 影子备份从来不清理`.
...
//! 4. `安装包RollbackService`
//!    (v0.9.21 RollbackService.js, obfuscated, 1:1 翻译源)
...
///
/// v0.9.21单影子 800 MB+ 是常态 (per 71GB / 91 = ~780 MB 平均).
...
///
/// v0.9.21无总大小上限, 所以 71GB 持续累积直到磁盘满.
...
///
/// v0.9.21用字符串 `'full' / 'file' / 'diff' / 'git' / 'session' / 'auto'`;
...
///
/// v0.9.21 `RollbackService.js` 的 4 主方法 + 6 策略.
...
///
/// v0.9.21平均影子 780 MB, 单影子无上限, 所以 91 个影子直接 71GB.
