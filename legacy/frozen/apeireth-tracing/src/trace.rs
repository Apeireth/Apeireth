//! # Trace
//!
//! `Trace` 是 distributed tracing 的根容器, 1:1 翻译 v0.9.21
//! `tracing` 的 Trace + Tracer.
//!
//! ## 字段
//!
...
use crate::exporter::{build_exporter, SpanExporter};
use crate::propagation::is_valid_span_id;
use crate::propagation::is_valid_trace_id;
...
mod tests {
    ...
    async fn test_sampled_false_no_export() {
        let tmp = NamedTempFile::new().unwrap();
        let cfg = test_config_file(tmp.path().to_path_buf());
        let sampler = Arc::new(crate::sampler::AlwaysOffSampler);
        let mut t = Trace::new(cfg, sampler).await.unwrap();
        assert!(!t.sampled);
        t.start_root("op", SpanKind::Client).await.unwrap();
        t.end_span("op").await.unwrap();
