//! # apeireth-observability
//!
//! **Observability skeleton crate** — 1.0 release 12 项 checklist #8 observability 必做
//! (per `v09021-rust-translation-blueprint` §2.5.3).
//!
//! 1:1 翻译 v0.9.21 `out/main` 集成面 (per blueprint §2.5.3):
//! - tracing (per-request `trace_id` + `span_id` 注入, 跟 `tracing` 0.1 联动)
//! - metrics (Prometheus exposition format, counter / gauge / histogram 3 类)
//! - logging (structured JSON, 跟 tracing span 联动)
//! - health (3 端点: `/health` / `/ready` / `/metrics`)
//! - OTLP (R20 阶段 3 续, 阶段 1 skeleton 0 引 OpenTelemetry SDK)
//!
//! ## v0.9.21 实查
//!
//! 商业版 `*` + `out/main/agent/AgentMCPServer.js` 0 处 observability 集成
//! (搜 `opentelemetry` / `prometheus` / `jaeger` / `telemetry` / `metrics` / `tracing` / `observ` / `monitor` 全 0 命中).
//! 1.0 release 必须从蓝图 §2.5.3 自己设计 (基于 OpenTelemetry 工业标准), skeleton 阶段用 std tracing + 简单 JSON 日志.
//!
//! ## 2 重 PII 脱敏防御 (per P0 安全铁律 + 跟 keyring SecretBytes 一致)
//!
//! | # | 防御 | 落地方式 | 编译期保证 |
//! |---|------|---------|-----------|
//! | 1 | **regex 模式 hardcode** | `PII_REDACTION_PATTERN` 编译期 `&str`, 4 关键词 (password/secret/token/key) | 静态 fixture 验证 `p0_pii_redaction` |
//! | 2 | **redact_pii 静态调用** | lib.rs 实际代码行 (排除注释) `redact_pii` 调用 ≥1, 不允许凭空说"已脱敏" | fixture 静态扫描源码计数 |
//!
//! ## 6 哲学 anchor 穿透
//!
//! - **S-1 北极星导向**: 1:1 翻译 v0.9.21 `blueprint §2.5.3`, 0 业务重设计
//! - **S-2 实事求是**: 估 350-450 LOC, skeleton 阶段 std tracing + 简单 JSON (0 OpenTelemetry SDK), R20 阶段 3 续
//! - **O-2 走在前人肩上**: 借鉴 OpenTelemetry / Prometheus 工业标准, 不另立协议
//! - **O-3 干到底**: 10 编译期 hardcode + 8 工具白名单 + 6 fixture (4 K-1 + 1 P0 + 1 m3)
//! - **O-4 任何人都能接手**: §1-§6 跟 keyring/machine-id/lark 同骨架 + 引用 reports 完整 path
//! - **O-5 不假装**: 所有方法 `warn!` 占位 + 真实 trace_id/span_id 生成, 0 假装已接 OTLP collector
//!
//! ## 引用文档 (4 份)
//!
...
//!
//! ## 状态: ⚠️ skeleton (R20 阶段 1 实施, 主 2026-08-05 21:12 拍板"派 A", 1.0 release 必续)
...
// R25.2 估补: 9 器官拟人化仪表盘 + 3 端点 TUI 集成 (per 1.0 release #8 observability 100%).
// 必要小改 1 行 mod 声明 (跟 sister #1 借鉴 Golutra 9 器官 TUI command 同模式, per task spec).
...
pub use health::{health_check, render_health_response, HealthEndpoint};
pub use logging::{log_structured, LogEntry, LogLevel};
pub use metrics::{render_prometheus, MetricsRegistry};
pub use tracing_integration::{next_trace_id, trace_span, TraceContext};
