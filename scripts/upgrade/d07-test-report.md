...
| Shim 路径 | `` (sqlite3/psql/systemctl 各一个 `exit 0` shim) |
...
- 本次测试必须在 `` 建 3 个 `exit 0` shim 才能跑通, 任何真实 CI 环境都会卡在这一步
...
|----|--------|-----|------------------|
| **S-1** | 22:33 | 规范是底 | ✅ 严格按 f5c44769 commit 注释 + 内部规范/§3.7 跑, 8 步 + 5 步都跟规范对齐 |
...
| shim 自身 | `{sqlite3,psql,systemctl}` |
