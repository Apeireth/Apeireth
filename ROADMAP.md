...
| P9 | 桌面功能面接全（能力旋钮 UI + 内省视图） | 📋 计划（2026-09-10 接面对账登记）：① Settings 增加"高级能力"开关区——shell/fetch/organs/preference_learning/judge/council → `APEIRETH_ENABLE_*`/`APEIRETH_COGNITIVE_*` 注入侧车（复用 P0-A apply 管道，默认全关 fail-closed；shell/fetch 开启后仍走人工审批）；② `/v1/organs` + `/v1/modules` 器官观察挂活动视图（后端已有、零消费方）；③ `/v1/providers` + `/v1/runtime/snapshot` 内省接入设置/健康面板（当前设置页 provider 预设为前端硬编码）。审计基线：聊天/记忆/工具审批/事件/审计核心面已 100% 接入（2026-09-10 实测对账） | 管道已建（2026-09-10，commit `1a265600`） |
